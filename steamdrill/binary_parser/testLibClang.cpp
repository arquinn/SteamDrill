...
//#include "../protos/steamdrill.ph.h"
...
boost::optional<Parser::FunctionDefinition>
getFunctionDefinition(CXCursor func)
{
...
int main(int argv, char **argc)
{
  ...
  std::cout << "Getting functions from : " << argc[1] << std::endl;
  ...
  std::string indent = "";
  std::vector<Parser::FunctionDefinition> funcs;
    ...
    [](CXCursor c, CXCursor parent, CXClientData client_data)
    {
      if (clang_getCursorKind(c) == CXCursor_FunctionDecl ) {
        auto funcs = static_cast<
          std::vector<Parser::FunctionDefinition>*>(client_data);
        auto fd = getFunctionDefinition(c);
        if (fd)
