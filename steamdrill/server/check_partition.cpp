...
int main(int argc, char **argv) {
  ...
  auto vals = getParts("parts", args::get(replay), args::get(cores));
  for (int i = 0; i < args::get(cores); ++i) {
    std::cout << vals[i].start_pid << " " << vals[i].start_clock << " "
              << vals[i].stop_pid << " " << vals[i].stop_clock << " "
              << vals[i].ckpt_clock << std::endl;
  }
  return 0;
}
