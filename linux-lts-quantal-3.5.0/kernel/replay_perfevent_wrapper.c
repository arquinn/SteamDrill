...
static void
write_instructions (struct perfevent_sampler *wrapper)
{
...
static __u32
read_counter(struct perfevent_sampler *wrapper) 
{
	int rc;
	long long count;
...
static void
begin_it(struct perfevent_sampler *wrapper, struct replay_perf_it *it) 
{
	int rc;
...
static void
end_it(struct perfevent_sampler *wrapper, struct replay_perf_it *it) 
{
...
static int
it_has_data(struct perfevent_sampler *wrapper, struct replay_perf_it *it)
{
...
static void
next_it(struct perfevent_sampler *wrapper, struct replay_perf_it *it) {
	struct perf_event_header hdr;
...
static void
get_it(struct perfevent_sampler *wrapper, struct replay_perf_it *it){
 ...
 */
int
init_perfevent_sampler(struct perfevent_sampler *wrapper, char *logdir, unsigned int sample_type, unsigned int sample_config, unsigned int sample_period, unsigned int data_size)
{ 
...
void
destroy_perfevent_sampler(struct perfevent_sampler *wrapper)
{
...
void
perfevent_sampler_start(struct perfevent_sampler *wrapper)
{
	int rc;
	mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);
...
void
perfevent_sampler_stop(struct perfevent_sampler *wrapper)
{
	int rc;
	mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);
...
void
perfevent_sampler_iterate(struct perfevent_sampler *wrapper)
{
	struct replay_perf_it it;
	__u32 inst;

	perfevent_sampler_stop(wrapper);
	...
	end_it(wrapper, &it);
	perfevent_sampler_start(wrapper);
}


int init_perfevent_counter(struct perfevent_counter** counter,
                           ...
                           unsigned int counter_value)
{
	...
	set_fs(KERNEL_DS);
	*counter = kmalloc(sizeof(struct perfevent_counter), GFP_KERNEL);
        printk("init perfevent 0x%lx\n", (u_long)*counter);
	...
	pe.disabled = 0; //Maybe this will work?
	pe.exclude_kernel = 1;
	pe.exclude_hv = 1;
	(*counter)->perf_fd = sys_perf_event_open(&pe,
                                                  current->pid,
                                                  -1,
                                                  -1,
                                                  0);
	if ((*counter)->perf_fd < 0) {
		printk(
                    "Pid %d init_perfevent_counter: cannot open %d\n",
                    current->pid,
                    (*counter)->perf_fd);
                return -EINVAL;
        }

        rc = sys_ioctl((*counter)->perf_fd, PERF_EVENT_IOC_RESET, 0);
        if (rc) {
                printk("Pid %d couldn't reset new perfevent_counter\n",
                       current->pid);
        }
        set_fs(old_fs);
        return 0;
...
void destroy_perfevent_counter(struct perfevent_counter *counter)
{
        /*
          is anything terrible going to happen if I don't close?
        mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);

        printk("destroying perfevent 0x%lx\n", (u_long)counter);
        if (counter->perf_fd > 0) {
                sys_close(counter->perf_fd);
        }
        set_fs(old_fs);
        */
        kfree(counter);
...
void perfevent_counter_start(struct perfevent_counter *counter)
{
        int rc;
	mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);
	rc = sys_ioctl(counter->perf_fd, PERF_EVENT_IOC_ENABLE, 0);
        if (rc) {
                printk("Pid %d couldn't start perfevent_counter\n",
                       current->pid);
        }
	set_fs(old_fs);
}

void perfevent_counter_stop(struct perfevent_counter *counter)
{
        int rc;
	mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);
	rc = sys_ioctl(counter->perf_fd, PERF_EVENT_IOC_DISABLE, 0);
        if (rc) {
                printk("Pid %d couldn't stop perfevent_counter\n",
                       current->pid);
        }
	set_fs(old_fs);
}

long long perfevent_counter_getval(struct perfevent_counter *counter)
{
        int rc;
        long long rtn;
        mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);
        ...
        rc = sys_read(counter->perf_fd, (char *)&rtn, sizeof(rtn));
        if (rc < 0)
        {
                printk("Pid %d: perfevent_counter_getval cannot read %d\n",
                       current->pid,
                       rc);
                rtn = rc;
        }
