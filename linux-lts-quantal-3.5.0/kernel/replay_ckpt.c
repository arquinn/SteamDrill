...
struct ckpt_proc_data {
	...
	u_long p_pthread_status;
	u_long p_libc_pthread_status;
	u_long timecnt;
	u_long prev_timepos;
	s64    time;	
	u_long user_timecnt;
	u_long user_prev_timepos;
	s64    user_time;
...
static int 
get_dev_zero_mmap (struct btree_head32 *replay_mmap_btree, int key, char *filename)
{ 	
	sprintf(filename, WRITABLE_MMAPS, key);
	return 1;
}
...
replay_full_checkpoint_proc_to_disk (char* filename, struct task_struct* tsk, pid_t record_pid, 
				     ...
				     u_long replay_hook, u_long pthread_status, u_long libc_pthread_status,
				     u_long timecnt, u_long prev_timepos, s64 time, 
				     u_long user_timecnt, u_long user_prev_timepos, 
				     s64 user_time, 
				     loff_t* ppos)
{
	mm_segment_t old_fs = get_fs();
	...
	cpdata.p_pthread_status = pthread_status;
	cpdata.p_libc_pthread_status = libc_pthread_status;
	cpdata.timecnt = timecnt;
	cpdata.prev_timepos = prev_timepos;
	cpdata.time = time;
	cpdata.user_timecnt = user_timecnt;
	cpdata.user_prev_timepos = user_prev_timepos;
	cpdata.user_time = user_time;
	...
	if (!is_thread) { 
		...
		for (vma = tsk->mm->mmap; vma; vma = vma->vm_next) {
			...
			if (!(pvmas->vmas_flags & VM_READ)) {
				printk("ckpt: skipping non-readable %s (%lx - %lx)\n", 
				       pvmas->vmas_file,
				       pvmas->vmas_start,
				       pvmas->vmas_end);
				continue;
			...
			if ((pvmas->vmas_flags&VM_MAYSHARE) && 
			    strncmp(pvmas->vmas_file, "/dev/zero", 9) && 
			    strncmp(pvmas->vmas_file, WRITABLE_MMAPS, WRITABLE_MMAPS_LEN))
			{ 
				//skip other shared things? 
				printk("ckpt: skipping shared %s (%lx - %lx)\n", 
				       pvmas->vmas_file, 
				       pvmas->vmas_start, 
				       pvmas->vmas_end);
...
long replay_full_resume_proc_from_disk (char* filename, pid_t clock_pid, int is_thread, 
					...
					u_long *child_tid,u_long *replay_hook, u_long *pthread_status,
					u_long *libc_pthread_status, u_long *timecnt,
					u_long *timepos, s64 *time, 
					u_long *user_timecnt,
					u_long *user_timepos, s64 *user_time, 
					int    zero_key,
					loff_t* ppos)
{
	mm_segment_t old_fs = get_fs();
	...
	*pthread_status = cpdata.p_pthread_status;
	*libc_pthread_status = cpdata.p_libc_pthread_status;
	*timecnt = cpdata.timecnt;
	*timepos = cpdata.prev_timepos;
	*time = cpdata.time;
	*user_timecnt = cpdata.user_timecnt;
	*user_timepos = cpdata.user_prev_timepos;
	*user_time = cpdata.user_time;
...
*/
					...
					if (!strncmp(pvmas->vmas_file, "/dev/zero", 9)) {
						get_dev_zero_mmap(&replay_mmap_btree, zero_key, pvmas->vmas_file);
						flags = O_CREAT|O_RDWR;					
						map_file = filp_open (pvmas->vmas_file, flags, 0777);
						if (IS_ERR(map_file)) {
							rc = PTR_ERR(map_file);
						...
						rc = add_sysv_shm(current->replay_thrd,
								  (u_long)pvmas->vmas_start, 
...
//			}
			...
			if (!(pvmas->vmas_flags&VM_READ) || 
			    ((pvmas->vmas_flags&VM_MAYSHARE) && 
			     strncmp(pvmas->vmas_file, "/dev/zero", 9) && 
			     strncmp(pvmas->vmas_file, WRITABLE_MMAPS, WRITABLE_MMAPS_LEN))) {
				//printk("skipping read %s (0x%lx,0x%lx) %lld\n", pvmas->vmas_file, pvmas->vmas_start, pvmas->vmas_start + (pvmas->vmas_end - pvmas->vmas_start), *ppos);

				continue;  // Not in checkpoint - so skip writing this one
			}				
			if (!(pvmas->vmas_flags&VM_WRITE)){
                                // force it to writable temporarilly
				rc = sys_mprotect (pvmas->vmas_start, pvmas->vmas_end - pvmas->vmas_start, PROT_WRITE); 
			}
		     

			MPRINT("%d reading from ckpt file into (0x%lx,0x%lx) %lld\n", current->pid, pvmas->vmas_start, pvmas->vmas_start + (pvmas->vmas_end - pvmas->vmas_start), *ppos);
			...
			if (!(pvmas->vmas_flags&VM_WRITE)) 
				rc = sys_mprotect (pvmas->vmas_start, pvmas->vmas_end - pvmas->vmas_start, pvmas->vmas_flags&(VM_READ|VM_WRITE|VM_EXEC)); 
		}
