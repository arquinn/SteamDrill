	...
	__acquires(&current->sighand->siglock)
{
	...
	if (may_ptrace_stop()) {
		...
		schedule();
		// how does this work for nginx's forked processes...?
			 ...
			 struct pt_regs *regs, void *cookie)
{
	 ...
	 */
        //if (likely(check_skipper(current->pid, regs->ip)))
        //{
        current->jobctl |= JOBCTL_STOP_DEQUEUED;
        ptrace_stop(signr, CLD_TRAPPED, 0, info);
                ...
                //else
                //{
                // if we skip a check, set the exit_code to 0 automatically
                //current->exit_code = 0;
                //        }
		...
		if (signr == 0) {
			if (current->record_thrd && replay_has_pending_signal()) {
				...
				if (current->replay_thrd && signr) {
					...
					if (signr == 17)
						printk ("sigchild pid %d, status %d\n",
...
#ifdef REP_SIG_DEBUG
			if (current->replay_thrd)
				printk("Replaying pid %d ptrace_signal before signr %d \n", current->pid, signr);
#endif
//end REPLAY
...
#ifdef REP_SIG_DEBUG
			if (current->replay_thrd)
				printk("Replaying pid %d did unlikely ptrace_signal signr %d \n", current->pid, signr);
#endif
//end REPLAY
			if (!signr)
				continue;
		...
		if (current->record_thrd && signr > 0) {
#ifdef REP_SIG_DEBUG
			printk("record thread gets sig %d\n", signr);
#endif
			if (!(signr == 9 && SI_FROMKERNEL(info))) {
