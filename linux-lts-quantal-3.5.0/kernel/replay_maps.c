...
int add_file_to_cache (struct file* vm_file, const char *orig_filename, dev_t* pdev, unsigned long* pino, struct timespec* pmtime) 
{
        // oname might need bonus digits
	char cname[CACHE_FILENAME_SIZE], nname[CACHE_FILENAME_SIZE], tname[CACHE_FILENAME_SIZE], oname[CACHE_FILENAME_SIZE];
	struct stat64 st;
	int fd, rc, copyed;
	mm_segment_t old_fs;
	...
	struct kstat stat;
	int mode;
        size_t orig_filename_len = strnlen(orig_filename, PATH_MAX);

	*pino = inode->i_ino;
	*pdev = inode->i_sb->s_dev;
	sprintf (cname, "%s/%x_%lx", cache_dir, inode->i_sb->s_dev, inode->i_ino);
        sprintf (oname, "%s/%x_%lx.orig", cache_dir, inode->i_sb->s_dev, inode->i_ino);
	...
	if (rc >= 0) {
		...
		if (inode->i_mtime.tv_sec < st.st_mtime ||
			...
			rc = sys_rename (cname, nname);
                        ...
                        // copy my file mapping file
                        sprintf (nname, "%s/%x_%lx_%lu_%u.orig", cache_dir, inode->i_sb->s_dev, inode->i_ino, st.st_mtime, st.st_mtime_nsec);
			rc = sys_rename (oname, nname);
	...
	if (rc < 0) printk ("add_file_to_cache: atomic rename from %s to %s failed, rc = %d\n", tname, cname, rc);
        ...
        fput(ofile);
        sys_close(fd);
        DPRINT("%s is post rename. now creating the mapping file\n");

        // add in the mapping file:
	sprintf (tname, "/%s/cf_%d.orig\n", cache_dir, current->pid);
	fd = sys_open (tname, O_CREAT|O_TRUNC|O_WRONLY, mode);
	if (fd < 0) {
		DPRINT ("add_file_to_cache: cannot create cache file %s, rc=%d\n", cname, fd);
		set_fs(old_fs);
        ...
        opos = 0;
        ofile = fget(fd);
        rc = vfs_write(ofile, orig_filename, orig_filename_len, &opos);
        if (rc != orig_filename_len) {
                printk("tried to write %s (%d), only wrote %d\n",
                       orig_filename, strnlen(orig_filename, PATH_MAX), rc);
                fput (ofile);
                sys_close (fd);
                rc = sys_unlink (tname);
                if (rc < 0) printk ("add_file_to_cache: cannot delete cache file on error\n");
        ...
        rc = sys_rename(tname, oname);
	fput(ofile);


        // update stats on newly created cache file
        rc = sys_stat64 (cname, &st);
...
int get_cache_file_info (dev_t dev, u_long ino, struct timespec mtime, struct timespec *pnmtime)
{
	...
	if (mtime.tv_sec < st.st_mtime || (mtime.tv_sec == st.st_mtime && mtime.tv_nsec < st.st_mtime_nsec)) {
                // that was the correct time
                pnmtime->tv_sec = st.st_mtime;
                pnmtime->tv_nsec = st.st_mtime_nsec;
	}
        else {
                // set to the input pnmtime... better hope that it is correct (!)
                pnmtime->tv_sec = mtime.tv_sec;
                pnmtime->tv_nsec = mtime.tv_nsec;
        }
	set_fs(old_fs);
