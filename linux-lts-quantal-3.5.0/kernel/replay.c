/*
  ...
  Jason Flinn
  Ed Nightingale
  Andrew Quinn   // think anyone will notice :)
*/
...
#include "replay_perfevent_wrapper.h"
...
//MADE THESE WAYYYYYYY LONGER
#define SCHED_TO 5000000
#define WAIT_TO 5000000
...
my_KMALLOC(size_t size, gfp_t flags, int line)
{
	...
	printk ("%d: %d: allocated: %p: %u\n", 
		current->pid, line, ptr, size);
	return ptr;
}
...
{
	printk ("%d: %d: freeing: %p\n", 
		current->pid, line, ptr);
...
static void __inode_data_put(struct inode_data *idata) {
	...
	// double check that refcount is 0...
	// fixes an atomicity issue between get and put
	if (atomic_read(&idata->refcnt)) {
...
unsigned long record_timings_flag = 0;
unsigned long skip_write_timings = 0;
unsigned long timing_buffer_size = 10000;
...
unsigned int ptrace_use_dev_null = 0;
...
// Info to store system call inputs (optimization vs. replay in some cases)
...
// this seems too big to get from GFP_KERNEL..? maybe its 32-40 bytes per?
#define SYSCALL_INPUT_SIZE (0x1000 * 256)
struct syscall_inputs {
        char *element;
        char *start_element;
        char *end_element;
        char log_created;
...
void init_syscall_inputs(struct syscall_inputs **sysin);
void dest_syscall_inputs(struct syscall_inputs *sysin);
void add_syscall_inputs(struct syscall_inputs *sysin, const struct mmap_input* element);
void write_syscall_inputs(struct syscall_inputs *sysin);
...
struct record_group {
	...
	int finished;        //Is the replay group finished running?

        // since fds are allocated sequentially, this is OK:
};
...
struct replay_group {
	...
	u_long rg_exit_clock;        // clock value to exit on if early term
	...
	u64    rg_elapsed;          // number of ns that have elapsed during replay group
	...
	struct xray_monitor* rg_open_socks; // Keeps track of open sockets for partitioned replay
        struct perfevent_sampler rg_perf_sampler; // Estimates stats surrounding replay behavior (for partitioning)

        // synchronization for attaching (why does this come up now...?)
        wait_queue_head_t restart_queue;
        atomic_t restart_count;

        wait_queue_head_t attached_queue; // the queue of tasks waiting for this replay to finish
        int attached;        //Is a device attached to the replay group?
	int ignore_segfaults; //Should we output segfault warnings?

        int active_tracers; // flag for letting the kernel know that there is an 'active' tracer in the ptrace tool
        int watchdog; // if > 0, there is a watchdog on this replay
        u_long report_syscall_clock; // when to start reporting systemcalls
};
...
#define RECORD_TIMEBUF_ENTRIES 10000
...
#define RECORD_FILE_SLOTS 1024
	...
	struct record_timing* rp_timebuf; // Buffer for recording timings (if requested)
	u_int rp_timecnt; // number of entries in the rp_timebuf
	loff_t rp_timepos; // Write postition in timings file
	loff_t rp_prev_timepos; //The previous timepos (for checkpointing)
	int rp_no_timebuf;

        // info about syscall inputs in this recording
        struct syscall_inputs * rp_syscall_inputs;


	//stuff for user-level timing log:
	u_long rp_pthread_timing_log_addr;
	u_long rp_pthread_timing_log_opened;
	loff_t rp_pthread_timing_log_pos;
};
...
#define REPLAY_PTRACE_SYSCALL_NONE 0
#define REPLAY_PTRACE_SYSCALL_IGNORE 1
...
struct ptrace_output {
	int fd;
        int open_fd;
        int temp_fd;
	char output_name[MAX_PTRACE_OUTPUT_NAME];
	struct list_head list;
};

struct ptrace_temp_fd_mapping {
	int orig_fd;
	int temp_fd;
	struct list_head list;
};
...
struct replay_thread {
	...
	u_long app_syscall_addr;       // Address in user-land that is set when the syscall should be replayed
        ...
        int __user *ptrace_syscall_addr;     //tells us to ignore systemcalls (used by ptrace tools)
        u_long ptrace_syscall;          //tells us to ignore systemcalls (used for initialization)

        int __user **ptrace_pthread_status; // for delayed setting of the ptrace tools' pthread_status
        int __user **ptrace_libc_pthread_status; // for delayed setting of the ptrace tools' libc_pthread_status
	...
	int __user *rp_pthread_status;  // Points to the pthread_log_status in the user-level code
	int __user *rp_libc_pthread_status; // points to the pthread_log_status in libc (instead of libpthread)
	...
	struct munmap_attach_parms * rp_pin_attach_redo_munmap; //save params if we need to reattach after munmap
        ...
        struct redo_attach_parms *rp_pin_attach_redo_parms; // save parameters for a reattached systemcall
	...
	u_long rp_dumbass_link;  // pointer to dumbass link location when init'd during ptrace

	int rp_pthread_old_status; // added so that we can get pthread_status to be skip w/o having to init the whole log
	struct list_head rp_ptrace_outputs ; //used to allow the ptrace tool to write output to pre-existing replay fds. 
	struct list_head rp_ptrace_temp_fd_mappings; // allows ptrace tool to temporariliy produce output
	...
	int first_ptrace;
        ...
        // all for the internal recording timelog:
	s64    rp_time;
	u_long rp_timebuf_next;

	//for the user timelog:
	struct record_timing *rp_user_timebuf;
	s64    rp_user_time;
	u_long rp_user_timebuf_next;
	int rp_no_user_timebuf;

	loff_t rp_user_timepos; // Write postition in timings file
	loff_t rp_user_timelog_size; // Write postition in timings file
	loff_t rp_user_prev_timepos; //The previous timepos (for checkpointing)

        struct task_struct *tsk; // back pointer to the task that this replay thread belongs to. USE WITH CAUTION

        int perf_fds[MAX_PERF_FDS]; //perf fds to reset on systemcall
        int __user *counters[MAX_COUNTERS]; // counters to reset on each sytemcall
        int perf_tick_counter; // special counter to count
        u64 perf_counter_sum;
};
...
void write_cache_file_mappings (struct record_group *prg);
...
static void delete_ptrace_outputs (struct replay_thread* prt);
#ifdef WRITE_ASYNC
...
static asmlinkage long replay_epoll_wait(int epfd, struct epoll_event __user *events, int maxevents, int timeout);
static asmlinkage long replay_read (unsigned int fd, char __user * buf, size_t count);


static void add_record_timing (struct record_thread* prect, u_long syscall, u_long clock);
static void update_record_timing (struct replay_thread *prept, u_long clock);
static int skip_and_read_record_timing (struct replay_thread *prept);


static void update_user_record_timing (struct replay_thread *prept, u_long clock);
static int skip_and_read_user_record_timing (struct replay_thread *prept);
...
static inline int
test_ptrace_syscall(void)
{
	struct replay_thread* prt = current->replay_thrd;
        return (prt->ptrace_syscall_addr && (*(prt->ptrace_syscall_addr)) == REPLAY_PTRACE_SYSCALL_NONE) ||
                        (!prt->ptrace_syscall_addr && prt->ptrace_syscall == REPLAY_PTRACE_SYSCALL_NONE);
}

static inline void
reset_perf_and_counters(void)
{
        struct replay_thread *prt = current->replay_thrd;
        int i = 0;
        ...
        BUG_ON(prt == NULL);
        set_fs(KERNEL_DS);
        while (i < MAX_PERF_FDS && prt->perf_fds[i] != 0) {
                int rc;
                rc = sys_ioctl(prt->perf_fds[i], PERF_EVENT_IOC_RESET, 0);
                BUG_ON(rc != 0);
                ++i;
        }
        set_fs(old_fs);

        if (prt->perf_tick_counter >= 0 ) {
                BUG_ON (prt->counters[prt->perf_tick_counter] == NULL);
                prt->perf_counter_sum += *(prt->counters[prt->perf_tick_counter]);
                MPRINT("reset_perf_and_counters -- curr count=%d sum=%lld\n",
                       *(prt->counters[prt->perf_tick_counter]),
                       prt->perf_counter_sum);
        }
        {
                i = 0;
                while (i < MAX_COUNTERS && prt->counters[i] != NULL) {
                        *(prt->counters[i]) = 0;
                        ++i;
                }
        }
...
static inline int is_perf_sampling(void) {
...
gdb_unlink(struct replay_thread* prt)
{
	prt->gdb_state = 0;
        ...
        // the world is complicated when it comes to gdb:
        //    some of the processes can be attached to gdb without the other processes being
        //    attached which is gross, but just how gdb works. I guess we have to work
        //    around it.
...
// forward declare:
void wake_up_replay_waiters(struct replay_group *prp_group);
void wake_up_attached_waiters(struct replay_thread *prp_thread);

void dec_restart_count(struct replay_thread *prt)
{
        struct replay_group *prepg = prt->rp_group;
        printk("%d: decrementing restart count\n", current->pid);
        //at this point, no, we weren't attached:
        // this will return true when prepg->restart_count == 0

        if (atomic_dec_and_test(&prepg->restart_count)) {
                wake_up_interruptible(&(prepg->restart_queue));
                printk("%d: woke up restart queue\n", current->pid);
        }
        else {
                printk("%d: testing restart_count? %d\n", current->pid, atomic_read(&prepg->restart_count));
        }
}
...
long 
set_other_location(struct task_struct *tsk, unsigned long location, int value)
{
	access_process_vm(tsk,location, (void *)&value, sizeof(int), 1);
	return 0;
}

long
get_other_location(struct task_struct *tsk, unsigned long location, int *value)
{
	access_process_vm(tsk, location, (void *)value, sizeof(int), 0);
	return 0;
}

void swap_pin_thread(struct replay_thread *to, struct replay_thread *from) {
        if (to->rp_pin_thread_data) {
                if (!to->tsk || !from->tsk) {
                        printk("cannot do our check! this mightfail if app is multithread+multiproc\n");
                }
                if (to->tsk && from->tsk && to->tsk->mm != from->tsk->mm) {
                        set_other_location (to->tsk, (unsigned long)(to->rp_pin_curthread_ptr), (int)to->rp_pin_thread_data);
                }
                else {
                        put_user (to->rp_pin_thread_data, to->rp_pin_curthread_ptr);
                }
        } else if (from->rp_pin_thread_data) {
                MPRINT ("Pid %d: I have pin thread data but switching thread %d (recpid %d) does not\n",
                        current->pid, to->rp_replay_pid, to->rp_record_thread->rp_record_pid);
                to->rp_pin_switch_before_attach = 1;
        }
}
...
set_replay_cache_file (struct replay_cache_files* pfiles, int fd, int cache_fd)
{
	...
	if (pfiles->data[fd] > 0) { 
		printk("set_replay_cache_file: problem setting %d to %d, already %d\n",
		       fd, cache_fd, pfiles->data[fd]);
	}
...
static void
clear_replay_cache_file (struct replay_cache_files* pfiles, int fd, int orig_fd)
{	
	if (fd < pfiles->count && pfiles->data[fd] == orig_fd)
		pfiles->data[fd] = -1;
}
...
new_replay_group (struct record_group* prec_group, int follow_splits)
{
	...
	prg->rg_timebuf = NULL;
	prg->rg_elapsed = 0;
	memset (&(prg->rg_perf_sampler), 0, sizeof(struct perfevent_sampler));
	...
	get_record_group (prec_group);
        ...
        // for the restart sychronization
        init_waitqueue_head(&(prg->restart_queue));
        //prg->restart_count = 0;
        atomic_set(&prg->restart_count, 0);


        //for the attached waiters api
	init_waitqueue_head(&(prg->attached_queue));
	prg->attached = 0;

	prg->ignore_segfaults = 0;
        prg->watchdog = 0;
        prg->report_syscall_clock = 0;
        prg->active_tracers = 0;
...
destroy_replay_group (struct replay_group *prepg)
{
	...
	if (is_pin_attached()) {
        ...
        if (prepg->rg_timebuf) KFREE(prepg->rg_timebuf);
	/*
	 ...
	 *          finished variable below.
      	 */

	wake_up_replay_waiters(prepg);
...
new_record_thread (struct record_group* prg, u_long recpid, struct record_cache_files* pfiles)
{
	...
	prp->rp_timecnt = 0;
	prp->rp_timepos = 0;
	prp->rp_prev_timepos = 0;
	prp->rp_no_timebuf = 0; //so that we don't continuously try to open a timelog that doesn't exist

	prp->rp_timebuf = NULL;

        // give these a default value:
        init_syscall_inputs(&prp->rp_syscall_inputs);


	prp->rp_pthread_timing_log_addr = 0;
	prp->rp_pthread_timing_log_opened = 0;
	prp->rp_pthread_timing_log_pos = 0;
		   ...
		   struct replay_cache_files* pfiles, struct replay_cache_files* pmfiles)
{
	...
	prp->ptrace_syscall_addr = NULL;
	prp->ptrace_syscall = REPLAY_PTRACE_SYSCALL_NONE;
        
        prp->ptrace_pthread_status = NULL;
        prp->ptrace_libc_pthread_status = NULL;
	...
	//for the ptraace tool
	prp->rp_pthread_status = NULL;
	prp->rp_pthread_old_status = 0;
	prp->rp_libc_pthread_status = 0;
	INIT_LIST_HEAD(&prp->rp_ptrace_outputs);
	INIT_LIST_HEAD(&prp->rp_ptrace_temp_fd_mappings);

	prp->rp_dumbass_link = 0;

	prp->fault_address = 0;
	prp->first_ptrace = 1;
	prp->rp_time = 0;
	prp->rp_timebuf_next = timing_buffer_size;

	prp->rp_user_timebuf = 0;
	prp->rp_user_time = 0;
	prp->rp_user_timebuf_next = 0;
	prp->rp_no_user_timebuf = 0;
	prp->rp_user_timepos = 0;
	prp->rp_user_timelog_size = 0;
	prp->rp_user_prev_timepos = 0;

	prp->is_pin_vfork = 0;
	prp->rp_pin_attaching = PIN_ATTACHING_NONE;
	prp->rp_pin_attach_redo_mmap = NULL;
	prp->rp_pin_attach_redo_munmap = NULL;
        prp->rp_pin_attach_redo_parms = NULL;
	...
	if (pmfiles) {
        ...
        // zero out the array
        memset((void*)prp->perf_fds, 0, sizeof(int) * MAX_PERF_FDS);
        memset((void*)prp->counters, 0, sizeof(int __user *) * MAX_COUNTERS);
        prp->perf_counter_sum = 0;
        prp->perf_tick_counter = -1;
...
__destroy_record_thread (struct record_thread* prp)
{
	...
	DPRINT ("       destroy_record_thread freeing log %p: end\n", prp->rp_log);
        ...
        // destroy stuff for the mem_mappigns
        dest_syscall_inputs(prp->rp_syscall_inputs);

	// KFREE the timebuf:
	if (prp->rp_timebuf != NULL)
		KFREE(prp->rp_timebuf);
...
__destroy_replay_thread (struct replay_thread* prp)
{
	...
	if (prp->rp_pin_attach_redo_mmap) KFREE (prp->rp_pin_attach_redo_mmap);
        ...
        if (prp->rp_pin_attach_redo_parms) KFREE (prp->rp_pin_attach_redo_parms);
	...
	prp->rp_group = NULL;
//	if (prp->rp_timelog != NULL)
//		VFREE(prp->rp_timelog); // ARQUINN: cleanup the timelog 
...
void ret_from_fork_replay (void)
{
	...
	if (is_gdb_fork_flagged()) {
		...
		if (!signalNumber) {
			printk("No signal was sent...? is gdb / ptrace not set to trace children?\n");
		}
		else {

			if (signalNumber != SIGSTOP) {
				//Something is very wrong
				printk("Replay pid %i is marked as from a gdb fork but didn't have a SIGSTOP queued. Something is wrong, kill it. signalNumber is %d\n",
				       current->pid, signalNumber);
			...
			MPRINT("Pid %d forked from gdb: notifying ptrace.\n", current->pid);
	...
	if (prg->rg_try_to_exit) {
        ...
        if (prg->watchdog) {
                MPRINT ("Killed by the dog! (signal fatal? %d)\n", fatal_signal_pending(current));
...
void init_syscall_inputs(struct syscall_inputs **sysin) {
        *sysin = (struct syscall_inputs*) VMALLOC(sizeof(struct syscall_inputs));
        (*sysin)->element =  VMALLOC(SYSCALL_INPUT_SIZE);
        (*sysin)->end_element = (*sysin)->element + SYSCALL_INPUT_SIZE;
        (*sysin)->start_element = (*sysin)->element;
        (*sysin)->log_created = 0;

        printk("%d: init_syscall_inputs %p %p\n", current->pid, (*sysin)->element);
}

void dest_syscall_inputs(struct syscall_inputs *sysin) {
        printk("%d: destroy sys inputs at %p %p\n", current->pid, sysin->element, sysin);
        VFREE(sysin->element);
        VFREE(sysin);
}
...
void add_syscall_inputs(struct syscall_inputs *sysin, const struct mmap_input *element) {
        ...
        u_long toAdd = sizeof(struct mmap_input);

        MPRINT("add_syscall_inputs @ %ld (%lx->%lx) %ld with %lx %lx %ld.%ld\n",
               element->clock, element->start, element->start + element->len, element->offset,
               element->dev, element->ino, element->mtime.tv_sec, element->mtime.tv_nsec);

        if (sysin->element + toAdd >= sysin->end_element) {
                MPRINT("dump sys_inputs from add\n");
                write_syscall_inputs(sysin);
        }

        BUG_ON(toAdd + sysin->element >= sysin->end_element);
        memcpy(sysin->element, element, toAdd);
        sysin->element += toAdd;
}
...
void write_syscall_inputs(struct syscall_inputs *sysin) {
	...
	set_fs(KERNEL_DS);
        ...
        MPRINT("write_syscall_inputs write %p -> %p\n", sysin->start_element, sysin->element);

	sprintf (filename, "%s/sysin.%d", prect->rp_group->rg_logdir, prect->rp_record_pid);
        if (sysin->log_created) {
                struct stat64 st;
		...
		ppos = st.st_size;
		fd = sys_open(filename, O_WRONLY|O_APPEND|O_LARGEFILE, 0777);
		MPRINT("Reopened log file %s, pos = %ld\n", filename, (long) ppos);

	} else {
		fd = sys_open(filename, O_WRONLY|O_CREAT|O_TRUNC|O_LARGEFILE, 0777);
		if (fd > 0) {
			rc = sys_fchmod(fd, 0777);
			if (rc == -1) {
				printk("Pid %d fchmod of syscall_input %s failed\n", current->pid, filename);
			}
		}
        ...
        if (fd > 0) {
                file = fget(fd);
                if (file) {
                        char *ptr = sysin->start_element;
                        while (ptr < sysin->element) {
                                MPRINT ("write %d from %p to %s\n", sysin->element - ptr, ptr, filename);
                                rc = vfs_write(file, ptr, sysin->element - ptr, &ppos);
                                if (rc < 0) {
                                        printk ("write_syscall_input got rc %d\n", rc);
                                        break;
                                }
                                else if (rc == 0) {
                                        printk("write_syscall_input go 0...?\n");
                                        break;
                                }
                                ptr += rc;
                        }
	...
	set_fs(old_fs);
        sysin->element = sysin->start_element;
}
...
static void dump_sysv_mappings(struct replay_thread *prt)
{
	struct sysv_mapping* tmp;
	list_for_each_entry (tmp, &prt->rp_sysv_list, list) {

		printk("sysv rcid %d -> rpid %d, key %d, size %d, flag %d\n",
		       tmp->record_id, 
		       tmp->replay_id, 
		       tmp->key, 
		       tmp->size, 
...
int add_sysv_shm(struct replay_thread *prt, u_long addr, u_long len) 
{ 
	struct sysv_shm* tmp;
	tmp = KMALLOC(sizeof(struct sysv_shm), GFP_KERNEL);
	if (tmp == NULL) {
		printk ("recpid %d: could not alllocate for sysv_shm\n", prt->rp_record_thread->rp_record_pid);
		return -ENOMEM;
	}
	
	tmp->addr = addr;
	tmp->len  = len;
	list_add(&tmp->list, &prt->rp_sysv_shms);
	return 0;
}

int find_and_delete_sysv_shm(u_long addr, u_long* len)
{
	int found = 0; //is it possible for size == 0 to be valid? 
	struct sysv_shm* tmp;
...
int copy_sysv_shm(struct  replay_thread *src, struct replay_thread *dest) 
{
	int ret = 0;
	struct sysv_shm* tmp;
	list_for_each_entry (tmp,  &src->rp_sysv_shms, list) {
		ret = add_sysv_shm(dest, tmp->addr, tmp->len);
		if (ret) return ret;
	}
	return ret;
}
...
int set_pin_address (u_long pin_address, u_long thread_data, u_long __user* curthread_ptr, int* attach_ndx)
{
	...
	if (prept) {
		MPRINT ("%d: set_pin_address pin address is %lx attaching %d status %d\n",
		...
		if (prept->rp_pin_switch_before_attach) {
			MPRINT ("switched before this attach so update current thread\n");
		...
		if (prept->rp_pin_attaching) {
			...
			if (prept->rp_pin_attach_redo_mmap || prept->rp_pin_attach_redo_munmap ||
                            prept->rp_pin_attach_redo_parms) {
				MPRINT ("Pid %d: Need to redo syscall on PIN restart\n", current->pid);
...
int update_attaching_status(pid_t pid)
{
	...
	struct replay_thread *prept = NULL;
	tsk = find_task_by_vpid(pid);
	if (tsk) {
		if (tsk->replay_thrd) {
			prept = tsk->replay_thrd;
			prept->app_syscall_addr = 0;
			MPRINT("%d: set app_syscall_addr %lu\n",pid, prept->app_syscall_addr);

			if (tsk->replay_thrd->rp_pin_attaching) {
				if (prept->rp_record_thread->rp_record_pid != prept->rp_group->rg_attach_pid) {
					MPRINT("%d is now PIN_ATTACHING_FF\n", pid);
					prept->rp_pin_attaching = PIN_ATTACHING_FF; // Still need to wait for the clock 
					return PIN_ATTACH_BLOCKED; // This thread will block
				} else {
					MPRINT("%d is now PIN_ATTACHING_RESTART\n", pid);
					prept->rp_pin_attaching = PIN_ATTACHING_RESTART;
	...
	printk ("update_attaching_status called for something that is not a replay process\n");
	return -EINVAL;
}
EXPORT_SYMBOL(update_attaching_status);
...
long get_log_tgid (void)
{
        ...
        {
                // we're a thread and doing getpid
                task = find_task_by_vpid(current->tgid);
                MPRINT("get_log_tgid: %d -> %d",
                       current->pid,
                       current->tgid);
        }
        if (!task->replay_thrd)
        {
		printk ("get_log_tgid called by a non-replay process\n");
		return -EINVAL;
        }
        return task->replay_thrd->rp_record_thread->rp_record_pid;
}
EXPORT_SYMBOL(get_log_tgid);


long get_log_id (pid_t pid)
{
	...
	if (tsk->replay_thrd) {
		return tsk->replay_thrd->rp_record_thread->rp_record_pid;
	} else {
...
replay_ckpt_wakeup (int attach_device, char* logdir, char* linker, int fd,
		    int follow_splits, int save_mmap, loff_t attach_index, 
		    int attach_pid, int ckpt_at, int record_timing,
		    ...
		    u_long exit_clock)
{
	...
	prepg->rg_exit_clock = exit_clock;
	...
	if (record_timing) {
		...
		if (replay_perf_sample) {
		        init_perfevent_sampler(&(prepg->rg_perf_sampler),
						 precg->rg_logdir,
						 replay_perf_sampling_type,
						 replay_perf_sampling_config,
						 replay_perf_sampling_period,
						 replay_perf_data_size);
			perfevent_sampler_start(&(prepg->rg_perf_sampler));
		}
	}

	if (ckpt_at > 0) prepg->rg_checkpoint_at = ckpt_at;
	printk("ckpt_at is %d, rg_checkpoint_at is %d, exit_clock at %lu\n",ckpt_at, prepg->rg_checkpoint_at, exit_clock);
		...
		if ((attach_pid < 0 || record_pid == attach_pid) && attach_index <= 0) {
			...
			if (attach_device == ATTACH_GDB) {
				prept->gdb_state = 1;
                        ...
                        rg_lock(prept->rp_group->rg_rec_group);
			wake_up_attached_waiters(prept);
                        rg_unlock(prept->rp_group->rg_rec_group);
...
replay_full_ckpt (long rc)
{
	...
	MPRINT("rec pid %d: pthread status %p, libc pthread status %p\n", 
	       current->replay_thrd->rp_record_thread->rp_record_pid, 
	       current->replay_thrd->rp_pthread_status, current->replay_thrd->rp_libc_pthread_status);
	...
	retval = replay_full_checkpoint_proc_to_disk (ckpt, current, 
						      ...
						      (u_long) current->replay_thrd->rp_pthread_status,
						      (u_long) current->replay_thrd->rp_libc_pthread_status,
						      (u_long) current->replay_thrd->rp_timebuf_next,
						      (u_long) current->replay_thrd->rp_record_thread->rp_prev_timepos,
						      current->replay_thrd->rp_time,
						      (u_long) current->replay_thrd->rp_user_timebuf_next,
						      (u_long) current->replay_thrd->rp_user_prev_timepos,
						      current->replay_thrd->rp_user_time,						      
						      &pos);
	...
	while ((tmp = ds_list_iter_next(iter)) != NULL) {	
		...
		if (tsk && tsk->mm && tsk != current) {
			...
			MPRINT("rec pid %d: pthread status %p, libc pthread status %p\n", 
			       prt->rp_record_thread->rp_record_pid, 
			       prt->rp_pthread_status, prt->rp_libc_pthread_status);
			...
			if(prt->rp_ckpt_pthread_block_clock){				
				...
				retval = replay_full_checkpoint_proc_to_disk (ckpt, tsk, prt->rp_record_thread->rp_record_pid, curr_ckpt_tsk->is_thread, 0,
									      ...
									      (u_long) prt->rp_pthread_status,
									      (u_long) prt->rp_libc_pthread_status,
									      (u_long) prt->rp_timebuf_next,
									      (u_long) prt->rp_record_thread->rp_prev_timepos,
									      prt->rp_time,
									      (u_long) prt->rp_user_timebuf_next,
									      (u_long) prt->rp_user_prev_timepos,
									      prt->rp_user_time,
...
replay_full_ckpt_wakeup (int attach_device, char* logdir, char* filename, char *linker, char* uniqueid, int fd, 
			 ...
			 u_long nfake_calls, u_long *fake_call_points, u_long exit_clock)
{
	...
	loff_t pos;
	int clock, i, is_thread = 1, zero_key = 0;
	...
	printk("Pid %d Replay start ckpt %s \n",current->pid, filename);
...
//HERE! please add in a check of that pkrecord_clock// 
	...
	if (num_procs > 1) {
		...
		zero_key = get_next_mmap_file();
		pckpt_waiter->zero_key = zero_key;
	...
	record_pid = replay_full_resume_proc_from_disk(ckpt, current->pid, is_thread,
						       ...
						       (u_long*)&prept->rp_pthread_status,
						       (u_long*)&prept->rp_libc_pthread_status,
						       (u_long*)&prept->rp_timebuf_next,
						       (u_long*)&prect->rp_timepos,
						       (s64*)&prept->rp_time,
						       (u_long*)&prept->rp_user_timebuf_next,
						       (u_long*)&prept->rp_user_timepos,
						       (s64*)&prept->rp_user_time,
						       zero_key,
						       &pos);

	MPRINT ("Pid %d gets record_pid %ld exp clock %ld\n", current->pid, record_pid, prept->rp_expected_clock);
	MPRINT("Pid %d (%ld) pthread status %p, libc pthread status %p\n", current->pid, record_pid, 
	       prept->rp_pthread_status, prept->rp_libc_pthread_status);
	...
	rc = skip_and_read_record_timing(prept);
	if (rc < 0)
	{
		//ignore the error (it doesn't really matter)
		printk ("couldn't read record_timings?\n");
	}

	rc = skip_and_read_user_record_timing(prept);
	if (rc < 0)
	{
		//ignore the error (it doesn't really matter)
		printk ("couldn't read record_timings?\n");
	...
	printk ("Pid %d replay_ckpt returns %ld\n", current->pid, retval);
...
replay_full_ckpt_proc_wakeup (char* logdir, char* filename, char *uniqueid, int fd, int ckpt_pos)
{
	...
	ds_list_iter_t* iter;
	char ckpt[MAX_LOGDIR_STRLEN+20];
	int found, is_thread = 1, zero_key;
	u_long cur_ckpts, consumed;
	u_char ch, ch2;

	printk("Pid %d ckpt proc start %s\n", current->pid, filename);
	...
	zero_key = pckpt_waiter->zero_key;
	...
	record_pid = replay_full_resume_proc_from_disk (pckpt_waiter->ckpt, pckpt_waiter->clock_pid,
							is_thread,&retval, &prect->rp_read_log_pos, 
...
//	mutex_unlock(&ckpt_mutex);
	...
	//printk ("Pid %d (%ld) replay_full_ckpt_proc_wakeup restarting syscall %d (pos %ld)  w/ expected clock %lu, pthread_block_clock %lu\n", 
...
new_syscall_enter (long sysnum)
{
	...
	if (record_timings_flag)
		add_record_timing (prt, sysnum, new_clock);
...
//	else if (signr > 0)  //sloppy for now. I just want to see whats going on
//		return 0;
...
*/
	...
	if (sig_fatal(current,signr) && sysnum != psr->sysnum && sysnum != 0 /* restarted syscall */) {
		...
		printk ("Pid %d: after signal, user code will not run again, so the kernel needs to insert a fake call for replay sysnum %d\n", current->pid, sysnum);
		...
		if (pnext) {
			...
			pnext += sizeof(u_long);
                        ...
                        // tells us that we need a watchdog
                        if (sysnum < 0) need_fake_calls = WATCHDOG_COUNT;
	...
	if (ka->sa.sa_handler > SIG_IGN) {
		...
		printk("Pid %d: saving repsignal_context %p\n", current->pid, pcontext);
...
replay_signal_delivery (int* signr, siginfo_t* info)
{
	...
	if (!prt->rp_signals) {
		MPRINT ("pid %d replay_signal called but no signals, signr is %d, signo %d, errno %d, code %d clock %ld\n", current->pid, *signr, info->si_signo, info->si_errno, info->si_code, *(prt->rp_preplay_clock));
		*signr = 0;
		if (info->si_signo == 7)
		{
			MPRINT("pid %d signr 7: %p\n", current->pid, info->_sifields._sigfault._addr);
		}
...
write_user_log (struct record_thread* prect)
{
	...
	{
		rc = vfs_write(file, start + written, to_write - written, &prect->rp_read_ulog_pos);
		if (rc < 0) 
		{
			printk ("%d:write_user_log1: got %ld, tried to write %ld from %p\n", 
				current->pid, rc, to_write - written, start + written );
			rc = -EINVAL;
		}
		else if (rc == 0) 
		{
			printk("%d:write_usr_log2: tried to write %ld, got rc = 0\n", 
			       current->pid, to_write - written);
			break;
		}
		written += rc;		
	}
...
read_user_log (struct record_thread* prect)
{
	...
	// this is just lazy. plain old lazy! 
	copyed = 0;
	while (copyed < num_bytes)
	{
		// I'm not sure how this uses the same position as the one at 5417...? am I missing something?
		rc = vfs_read (file, 
			       (char __user *) (start + copyed), 
			       num_bytes - copyed, 
			       &prect->rp_read_ulog_pos);
		if (rc < 0)
		{
		...
		MPRINT ("read_user_log: tried to read num_bytes %d, copyed %ld, rc %ld\n", num_bytes, copyed, rc);
		copyed += rc; 
		if (rc == 0)
		{
			printk ("vfs_read returning 0...? starting at %llu\n", prect->rp_read_ulog_pos);
			break;
		}
...
long
write_user_timing_log (struct record_thread* prect)
{
	struct pthread_timing_log_head __user * phead = 
		(struct pthread_timing_log_head __user *) prect->rp_pthread_timing_log_addr;
	...
	MPRINT ("Pid %d: write_pthread_timing_log %p\n", current->pid, phead);
	if (phead == 0) return 0; // Nothing to do
	...
	MPRINT ("Pid %d: log current address is at %lx\n", current->pid, next); 
	start = (char __user *) phead + sizeof (struct pthread_timing_log_head);
	to_write = (char *) next - start;

	MPRINT ("Pid %d - need to write %ld bytes of pthread_timing log\n", current->pid, to_write);
	if (to_write == 0) {
		MPRINT ("Pid %d - no entries to write in pthread_timing log\n", current->pid);
		return 0;
	}

	sprintf (filename, "%s/utimelog.id.%d", prect->rp_group->rg_logdir, prect->rp_record_pid);
	
	old_fs = get_fs();
	set_fs(KERNEL_DS);

	// see if we're appending to the user log data before
	if (prect->rp_pthread_timing_log_opened) {
		MPRINT ("Pid %d, ulog %s has been opened before, so we'll append\n", current->pid, filename);
		rc = sys_stat64(filename, &st);
		if (rc < 0) {
			printk ("Pid %d - write_log_data, can't append stat of file %s failed\n", current->pid, filename);
			return -EINVAL;
		...
		prect->rp_pthread_timing_log_opened = 1;
		rc = sys_stat64(filename, &st);
	}
	...
	while (written < to_write)
	{
		rc = vfs_write(file, start + written, to_write - written, &prect->rp_pthread_timing_log_pos);
		if (rc < 0) 
		{
			printk ("write_pthread_timing_log1: tried to write %ld, got rc %ld\n", 
				to_write - written, rc);
			rc = -EINVAL;
		}
		else if (rc == 0) 
		{
			printk("write_pthread_timing_log2: tried to write %ld, got rc = 0\n", 
			       to_write - written);
			break;
		}
		written += rc;		
	...
	// We reset the next pointer to reflect the records that were written
	// In some circumstances such as failed execs, this will prevent dup. writes
	next = (u_long) phead + sizeof (struct pthread_timing_log_head);
...
{
	struct replay_thread* tmp, *min_rt = NULL;
	long retval = 0;
	int ret;

        // I think this needs to happen before I do anything rash
	...
	while (*(prt->rp_preplay_clock) < wait_clock_value) {
		...
		tmp = prt->rp_next_thread;
                ...
                // this iteration is a little bit different from what we normally do b/c we might be in a
                // restarted case, in which case another thread migth CURRENTLY be executing/finding a thread to run.
                // Soo... check for that:
                do {
			MPRINT ("Consider thread %d status %d clock %ld\n", tmp->rp_replay_pid, tmp->rp_status, tmp->rp_wait_clock);
                        if (tmp->rp_status == REPLAY_STATUS_RUNNING) {
                                BUG_ON (min_rt);
                                min_rt = tmp;
                                break;
                        }
                        else if (tmp->rp_status == REPLAY_STATUS_ELIGIBLE ||
			...
			tmp = tmp->rp_next_thread;
                ...
                if (min_rt->rp_status != REPLAY_STATUS_RUNNING) {
                        tmp->rp_status = REPLAY_STATUS_RUNNING;
                        swap_pin_thread(tmp, prt);
...
static inline int should_finish(void)
{
	struct replay_thread *prpt = current->replay_thrd;
	if (!prpt)  {
		printk ("%d: cannot call should_finish w/out replay\n",
			current->pid);
		return 0;
	}

	return prpt->rp_group->rg_exit_clock > 0 &&
		(prpt->rp_group->rg_exit_clock <
		 *(prpt->rp_preplay_clock));
};
...
get_next_syscall_enter (struct replay_thread* prt, struct replay_group* prg, int syscall, char** ppretparams, struct syscall_result** ppsr)
{
	...
	if (prt->rp_pin_attaching == PIN_ATTACHING_FF || prt->rp_pin_attaching == PIN_ATTACHING_RESTART) {
		...
		printk ("%d: Rolling back %ld bytes of log data\n", current->pid, clock_adj);
		argsrestore (prt->rp_record_thread, clock_adj);
		if (prt->rp_pin_attaching == PIN_ATTACHING_RESTART) {
                        // maybe maybe maybe we wait on a waitq...?
                        // figre out how this works... works.. works
                        struct replay_group *prepg = prt->rp_group;

                        // I bet I'm holding the lock right here...????
                        printk ("%d: sleeping for restarters!\n", current->pid);
                        rg_unlock(prg->rg_rec_group);
                        wait_event_interruptible_timeout(prepg->restart_queue,
                                                         !atomic_read(&prepg->restart_count),
                                                         WAIT_TO);

                        prt->rp_pin_attaching = PIN_ATTACHING_NONE;
                        printk ("%d: rg_attach_pid is attached!\n", current->pid);
                        rg_lock(prg->rg_rec_group);
                }
	}
...
//		MPRINT("Replay pid %d, argsconsumed %lu, pclock %lu, start_clock %lu\n", current->pid, argsconsumed(prect) - sizeof(u_long), *pclock, start_clock);
		// this used to be a lame warning. Of course replays get up to 100 million.. (wait, of course? lol). 
		if (start_clock > 4000000000u) {
	...
	if (psr->flags & SR_HAS_STOP_CLOCK_SKIP) { // Nead to read this in exactly this order but use it later
		...
		MPRINT ("%d: Stop clock skip is %lu\n", current->pid, prt->rp_stop_clock_skip);
		argsconsume(prect, sizeof(u_long));
		...
		while (!(prt->rp_status == REPLAY_STATUS_RUNNING || (prt->rp_replay_exit && prect->rp_in_ptr == prt->rp_out_ptr+1))) {	
			...
			if (ret == -ERESTARTSYS) {
				...
				if (prt->rp_pin_attaching == PIN_ATTACHING) {
					...
					if (syscall == 192 || syscall == 256 || syscall == 3)
                                                prt->rp_saved_rc = retval;  // We need to return this thanks to PIN wierdness
	...
	// Try to pause for attach when we are actually executing and before syscall is done - called right before we increment replay clock
	{
		...
		if (rc < 0) {
			rg_unlock (prg->rg_rec_group);
			if (syscall == 192 || syscall == 3 || syscall == 256) prt->rp_saved_rc = retval;  // We need to return this thanks to PIN wierdness
			return rc;
		}
	}
	...
	if (prt->rp_pin_attaching == PIN_ATTACHING_FF && start_clock > prg->rg_pin_attach_clock) {
                prt->rp_pin_attaching = PIN_ATTACHING_NONE;
                printk("%d: attached, start_clock %ld, rp_pin_attach_clock %ld\n", current->pid, start_clock, prg->rg_pin_attach_clock);
                dec_restart_count(prt);
	}
	...
	// populate the record_timings
	if (record_timings_flag)
		update_record_timing(prt, *(prt->rp_preplay_clock));
...
get_next_syscall_exit (struct replay_thread* prt, struct replay_group* prg, struct syscall_result* psr)
{
	...
	while (*(prt->rp_preplay_clock) < stop_clock ||  prt->rp_status == REPLAY_STATUS_RESTART_CKPT) {
		...
		if (prt->rp_pin_attaching == PIN_ATTACHING_FF) {
			prt->rp_pin_attaching = PIN_ATTACHING_NONE;
                        printk("%d: done attaching on exit\n", current->pid);
                        dec_restart_count(prt);
		}
...
//			MPRINT ("Replay pid %d waiting for clock value %ld on syscall exit but current clock value is %ld\n", current->pid, stop_clock, *(prt->rp_preplay_clock));
			...
			if (ret == -ERESTARTSYS) {
				...
				if (is_pin_attached() && (psr->sysnum != 11 || psr->sysnum != 120)) {
                                        ...
                                        // line 1 of the block of messages
...
{
	if (current->replay_thrd->rp_group->rg_try_to_exit ||
            current->replay_thrd->rp_group->watchdog) return 0; // Just bail
...
{
	if (current->replay_thrd->rp_group->rg_try_to_exit ||
            current->replay_thrd->rp_group->watchdog) return; // Just bail
	get_next_syscall_exit (current->replay_thrd, current->replay_thrd->rp_group, psr);
}

static void allocate_rp_timebuf(struct record_thread *prect, int size)
{
	prect->rp_timebuf = KMALLOC(size, GFP_KERNEL);
	if (prect->rp_timebuf == NULL) printk ("%d: Cannot allocate timing buffer\n", prect->rp_record_pid);
}
...
void write_timings (struct replay_group* prepg)
{
	...
	if (prepg->rg_timepos) {
		fd = sys_open(filename, O_WRONLY|O_LARGEFILE, 0644);
	} else {
		fd = sys_open(filename, O_WRONLY|O_CREAT|O_TRUNC|O_LARGEFILE, 0644);
	}
	if (fd < 0) {
...
record_timings (struct replay_thread* prept, short syscall)
{
	...
	getnstimeofday(&ts);
	elapsed = timespec_to_ns(&ts) - prepg->rg_elapsed;
	prepg->rg_elapsed += (u64)elapsed;
	...
	prepg->rg_timebuf[prepg->rg_timecnt].index = prept->rp_record_thread->rp_count;
        // would be great to get start and stop clocks...? 
	prepg->rg_timebuf[prepg->rg_timecnt].syscall = syscall;
	prepg->rg_timebuf[prepg->rg_timecnt].ut = ut;
	prepg->rg_timebuf[prepg->rg_timecnt++].elapsed = elapsed;

	if (is_perf_sampling())
	{
                perfevent_sampler_iterate(&(prepg->rg_perf_sampler));
	}
...
void write_record_timings (struct record_thread* prect)
{
	char filename[MAX_LOGDIR_STRLEN+20];
	int fd, rc; 
	struct file* file = NULL;
	...
	sprintf (filename, "%s/time_log.%d", prect->rp_group->rg_logdir, prect->rp_record_pid);
	old_fs = get_fs();
	set_fs(KERNEL_DS);
	...
	if (fd < 0) {
		printk("Pid %d write_record_timings: could not open file %s, %d\n", current->pid, filename, fd);
		prect->rp_timecnt = 0;
		return;
	}
	file = fget(fd);

	to_write = prect->rp_timecnt*sizeof(struct record_timing);
	do {
		written = vfs_write (file, (char *)prect->rp_timebuf+copied, to_write-copied, &prect->rp_timepos);
		if (written <= 0) {
			printk ("write_record_timing: vfs_write returns %d\n", written);
			break;
		}
		copied += written;
	} while (copied < to_write);
	...
	fput(file);
	rc = sys_close (fd);
	if (rc < 0) printk ("write_record_timings: file close failed with rc %d\n", rc);
	set_fs(old_fs);
	prect->rp_timecnt = 0;
}
...
static int
read_record_timing (struct replay_thread *prept)
{
        char filename[MAX_LOGDIR_STRLEN+20];
	struct file* file;
	struct stat64 st;
	mm_segment_t old_fs; 
	int fd, rc, size = timing_buffer_size * sizeof(struct record_timing);
	...
	sprintf (filename, 
		 "%s/time_log.%d", 
		 prect->rp_group->rg_logdir, 
		 prect->rp_record_pid);

	prect->rp_prev_timepos = prect->rp_timepos;
	MPRINT ("Reading %s at pos %lld \n", filename, (long long) prect->rp_timepos);
	fd = sys_open(filename, O_RDONLY|O_LARGEFILE, 0644);
	if (fd < 0) {
		printk ("read_record_timing: cannot open log file %s, rc %d\n", filename, fd);
		return -EINVAL;
	}

	if (prect->rp_timebuf == NULL)
	{
		rc = sys_stat64(filename, &st);
		if (rc < 0) {
			printk ("Stat of file %s failed\n", filename);
		}
		else if (st.st_size < timing_buffer_size * sizeof(struct record_timing))
		{
			size = st.st_size;
		}

		allocate_rp_timebuf(prect, size);
		printk("%d: allocated timelog size %d %lld\n", 
		       prept->rp_replay_pid, size, st.st_size);
	}


	file = fget(fd);
	rc = vfs_read (file,
		       (char *) prect->rp_timebuf,
		       size,
		       &prect->rp_timepos);

	if (rc != size) {
		printk ("read_record_timign: read %d, expected %d\n", 
			rc, 
			size);
	}
...
static int
skip_and_read_record_timing (struct replay_thread *prept)
{
	if (record_timings_flag)
	{
//		while (prept->rp_timebuf_next > timing_buffer_size)
//		{
//			read_record_timing(prept);
//			prept->rp_timebuf_next -= timing_buffer_size;
//		}
		return read_record_timing(prept); //I don't think there's anything special here	
	}
	return 0;
}

static void 
update_record_timing (struct replay_thread *prept, u_long clock)
{	
	struct record_timing *timing;
	struct record_thread *prect = prept->rp_record_thread;
	// don't do anything w/ this on attach or on ckpt restart
	if (prept->rp_status == REPLAY_STATUS_RESTART_CKPT || 
	    prept->rp_pin_attaching == PIN_ATTACHING_FF)
	{
		MPRINT("Pid %d (%d): skipping upate_record_timing...?\n",
		       current->pid, prept->rp_record_thread->rp_record_pid);
		
		timing = prect->rp_timebuf + prect->rp_timecnt;
		MPRINT("timelog %p timing ptr %p\n", prect->rp_timebuf, timing);
		MPRINT("%d record_timing (%p) %lu %lld\n", current->pid, 
		       timing, timing->clock, timing->elapsed);

		return;
	}

	//so I don't fail infinitely... mostly for processes w/out a timelog
	if (prect->rp_no_timebuf)
		return;

	//we need to allocate! 
	if (unlikely(prect->rp_timebuf == NULL))
	{
		prept->rp_timebuf_next = 0;		
		if (read_record_timing(prept))
		{
			MPRINT("couldn't populate timelog?\n");
			prect->rp_no_timebuf = 1;
			return;
		}	
	}
	timing = prect->rp_timebuf + prept->rp_timebuf_next;
	MPRINT("timelog %p timing ptr %p\n", prect->rp_timebuf, timing);

	if (timing->clock != clock)
		printk("%d record pid %d record_timing %lu %lld\n", 
		       current->pid, prept->rp_record_thread->rp_record_pid, 
		       timing->clock, timing->elapsed);
	
	//this *might* get dicey later...? not sure. For now I think its true
	BUG_ON(timing->clock != clock);
	MPRINT("%d record_timing (%p) %lu %lld\n", current->pid, 
	       timing, timing->clock, timing->elapsed);
	
	prept->rp_time = timing->elapsed;

	prept->rp_timebuf_next++;
	
	if ( prept->rp_timebuf_next >= timing_buffer_size) {
		MPRINT ("Pid %d recpid %d reaches end of in-memory timeog\n", 
			current->pid, prept->rp_record_thread->rp_record_pid);
	...
	update_user_record_timing(prept, clock);
}

static void allocate_rp_user_timebuf(struct replay_thread *prept, int size)
{
	prept->rp_user_timebuf = KMALLOC(size, GFP_KERNEL);
	if (prept->rp_user_timebuf == NULL) 
		printk ("%d: Cannot allocate timing buffer\n", prept->rp_replay_pid);
}


static int
read_user_record_timing (struct replay_thread *prept)
{
        char filename[MAX_LOGDIR_STRLEN+20];
	struct file* file;
	struct stat64 st;
	...
	if (prept->rp_user_timelog_size > 0 &&
	    prept->rp_user_timepos == prept->rp_user_timelog_size)
	{
		// we can't read any more from the user timebuf. 
		return 0;
	}
	...
	prept->rp_user_prev_timepos = prept->rp_user_timepos;

	MPRINT ("Reading %s at pos %lld \n", filename, (long long) prept->rp_user_timepos);
	fd = sys_open(filename, O_RDONLY|O_LARGEFILE, 0644);
	if (fd < 0) {
		printk ("read_record_timing: cannot open log file %s, rc %d\n", filename, fd);
		return -EINVAL;
	...
	if (prept->rp_user_timebuf == NULL)
	{
		...
		allocate_rp_user_timebuf(prept, size);
		prept->rp_user_timelog_size = st.st_size;
	...
	file = fget(fd);
	rc = vfs_read (file,
		       (char *) prept->rp_user_timebuf,
		       size,
		       &prept->rp_user_timepos);

	if (rc != size) {
		printk ("%d (%d)read_user_record_timign: read %d, expected %d\n", 
			current->pid,
			prect->rp_record_pid,
			rc, 
			size);		
...
static int
skip_and_read_user_record_timing (struct replay_thread *prept)
{
	if (record_timings_flag)
	{
		//I don't think there's anything special here	
		return read_user_record_timing(prept);
	}
	return 0;
}

static void 
update_user_record_timing (struct replay_thread *prept, u_long clock)
{	
	struct record_timing *timing;
	int rc = 0;
	if (prept->rp_no_user_timebuf)
		return;

	//we need to allocate! 
	if (unlikely(prept->rp_user_timebuf == NULL))
	{
		prept->rp_user_timebuf_next = 0;	
		rc = read_user_record_timing(prept);
		if (rc < 0)
		{
			MPRINT("couldn't populate timelog?\n");
			prept->rp_no_user_timebuf = 1;
			return;
		
		}
		else if (!rc)
		{
			MPRINT("read_record_timing finished? But I've only just begun!?\n");
			prept->rp_no_user_timebuf = 1;
			return;
		
		}
		// bonus set for first instance		
		prept->rp_user_time = prept->rp_user_timebuf[0].elapsed;
	}
	timing = prept->rp_user_timebuf + prept->rp_user_timebuf_next;

	if (timing->clock > clock)
	{
		//do nothing, no update to give
		MPRINT("user_timing: no update %lu timing: (%lu, %lld)\n", clock, timing->clock, timing->elapsed);
		return;
	}
	else 
	{	
		do {
			prept->rp_user_time = timing->elapsed;
			prept->rp_user_timebuf_next++;

			if ( prept->rp_user_timebuf_next >= timing_buffer_size) {
				MPRINT ("Pid %d recpid %d reaches end of in-memory user_timeog @ %lu\n", 
					current->pid, prept->rp_record_thread->rp_record_pid, clock);
				MPRINT ("For reference, curr clock %lu\n", timing->clock);
				...
				{
					printk("read_record_timing finished @ %lu\n", timing->clock);
					prept->rp_no_user_timebuf = 1;
					return;					
				}
			}
			...
			timing = prept->rp_user_timebuf + prept->rp_user_timebuf_next;
		}while (timing->clock < clock);		
	}

	MPRINT("user_timing: %lu timing: (%lu, %lld)\n", clock, timing->clock, timing->elapsed);
}

inline u64 rdtimestamp(void)
{
...
static void
add_record_timing (struct record_thread* prect, u_long syscall, u_long clock)
{
	//struct timespec ts;
	s64 elapsed;
	//getnstimeofday(&ts);
	//elapsed = timespec_to_ns(&ts);
	elapsed = rdtimestamp();

	if (!skip_write_timings)
	{
		if (prect->rp_timebuf == NULL)
			allocate_rp_timebuf(prect,
					    sizeof(struct record_timing) * timing_buffer_size);

		prect->rp_timebuf[prect->rp_timecnt].clock = clock;
		prect->rp_timebuf[prect->rp_timecnt++].elapsed = elapsed;

		if (prect->rp_timecnt == timing_buffer_size) write_record_timings (prect);
	}
}
	    ...
	    *(prept->rp_preplay_clock) == prepg->rg_attach_clock) { // Attach right before clock is incremented
		...
		if (prepg->rg_attach_device == ATTACH_PIN) {
			...
			for (tmp2 = prept->rp_next_thread; tmp2 != prept; tmp2 = tmp2->rp_next_thread) {
				if ((tmp2->rp_status == REPLAY_STATUS_RUNNING ||
				     tmp2->rp_status == REPLAY_STATUS_WAIT_CLOCK) && 
		...
		if (prepg->rg_attach_device == ATTACH_GDB) {
			prept->gdb_state = 1;
			// lets just try this and see wtf happens
			prept->rp_pin_attaching = PIN_ATTACHING;
                        // we ignore this one (don't increment restart_count) since it's the attach thread
                }
                // we already hold the lock b/c of... our callers caller *sigh*.
		wake_up_attached_waiters(prept);
		set_current_state(TASK_INTERRUPTIBLE);
                rg_unlock(prept->rp_group->rg_rec_group);
		schedule();
		...
		tmp = current->replay_thrd->rp_next_thread;
		while (tmp != NULL && tmp != current->replay_thrd) {
			printk("\t thread %d (recpid %d) status %d clock %ld\n",
			       tmp->rp_replay_pid,
			       tmp->rp_record_thread->rp_record_pid,
			       tmp->rp_status,
			       tmp->rp_wait_clock);
					tmp = tmp->rp_next_thread;
		}
...
//		if (prepg->rg_attach_device == ATTACH_PIN) {
		if (prepg->rg_attach_device) {
		    //ARQUINN: added logic to make it so pin_attaching only set for threads and not processes
			 ...
			 * tgid for the replay thread that is being woken up, then we need to set PIN_ATTACHING (
			 * this means that the threads are part of the same thread group).
			 */
			task = find_task_by_vpid(tmp->rp_replay_pid);
			...
			if( !task || (this_tgid == task->tgid )) {
                                ...
                                if (tmp->rp_status != REPLAY_STATUS_DONE) {
                                        ...
                                        // no need to lock this yet -- there's only one thread runing
                                        atomic_inc(&prepg->restart_count);
                                        printk("%d: restart_count to %d\n", tmp->rp_replay_pid, atomic_read(&prepg->restart_count));

                                        if (prepg->rg_attach_device == ATTACH_PIN)
                                                tmp->app_syscall_addr = 1;
                                        else if (prepg->rg_attach_device == ATTACH_GDB)
                                                tmp->gdb_state = 1;
...
get_next_syscall (int syscall, char** ppretparams)
{
	...
	// Allow the replay to bailout here.
	if (should_finish()) {
		try_to_exit(current->pid);
		printk("%d: replay finishing early\n", current->pid);
		sys_exit(0);
	}
	...
	if (tsk) {
		if (tsk->replay_thrd) {
			...
			if (cdone && tsk->replay_thrd->rp_group) {
				...
				if (tsk->replay_thrd->rp_group->rg_attach_device == ATTACH_GDB) {
					tmp = tsk->replay_thrd;
					do {
						if (tmp->gdb_state) {
							return tmp->rp_replay_pid;
				...
				//MPRINT("%d: get_attach_status rg_attach_device is none\n", pid);
				return 0;
			} else {
				MPRINT("%d: get_attach_status no replay group or no cdone %d\n", pid, cdone);
				return -EINVAL; // Not a replay task
			}
		}
		MPRINT("%d :get_attach_status not a replay thread\n", pid);
		return -EINVAL;
	} else {
		MPRINT("%d :get_attach_status not a task\n", pid);
		return -ESRCH;
	}
}
...
{
        //at this point, no, we weren't attached:
        struct replay_group *prepg = prp_thread->rp_group;
        // for stupid reasons, you cannot lock here (one of the callers already holds the lock)
        //rg_lock(prepg->rg_rec_group);
        prepg->attached = 1;
        wake_up_all(&(prepg->attached_queue));
        //rg_unlock(prepg->rg_rec_group);
        printk("woke up attachers for %d (%p)\n",current->pid,
               (void*)&(prepg->attached_queue));
}
...
pid_t wait_for_attach(pid_t pid)
{
	...
	if (tsk) {
		if (tsk->replay_thrd) {
		        cdone = atomic_read(&tsk->replay_thrd->ckpt_restore_done);
			prp_group = tsk->replay_thrd->rp_group;
			if (cdone && prp_group) {
                                printk("%d: found a replay task /w group %d!\n", current->pid, pid);
				rg_lock(prp_group->rg_rec_group);
				//at this point, no, we weren't attached:
				if (!prp_group->attached) {
                                        printk("%d: waiting on group for %d (%p)\n",
                                               current->pid, pid, (void*)&(prp_group->attached_queue));
				        rg_unlock(prp_group->rg_rec_group);
					wait_event_interruptible_timeout(prp_group->attached_queue,
									 prp_group->attached,
									 WAIT_TO);
                                        printk("%d: woken up by %d\n",current->pid, pid);
				    rg_lock(prp_group->rg_rec_group);
				}
				rg_unlock(prp_group->rg_rec_group);
				...
				tmp = tsk->replay_thrd;
				if (prp_group->rg_attach_device == ATTACH_PIN) {
					do {
						if (tmp->app_syscall_addr) {
							return tmp->rp_replay_pid;
						}
				...
				if (prp_group->rg_attach_device == ATTACH_GDB) {
					do {
						if (tmp->gdb_state) {
							return tmp->rp_replay_pid;
						}
				...
				return -EINVAL; //something bad happened

			} else {
                                printk("%d -- not replay task (yet)\n", pid);
				return -EINVAL; // Not a replay task
			}
		}
		return -EINVAL;
...
EXPORT_SYMBOL(wait_for_attach);


u_long get_prot_from_flags(unsigned long flags)
{
...
long copy_to_user_force(void *user_start, char *kernel, unsigned long size)
{
	long rtn, rtnProtects;
	unsigned long page, page_size;
	int i = 0, nPages;

	struct vm_area_struct *vma; 

	unsigned long orig_prots[0x10];

	// figure out the page size / page offset of this addr

	page = (long)user_start & PAGE_MASK;

	//get num bytes, then make it cover the whole last page:
	page_size = (((long)user_start + size) - page);
	page_size = (page_size / PAGE_SIZE + 1) * PAGE_SIZE;

	// huh... if page_size > 1, we need to handle multiple pages!
	nPages = (page_size / PAGE_SIZE );

	MPRINT("force write to %p,%lx (page %lx, %lx) nPages %d \n", user_start, size, page, page_size, nPages);
	BUG_ON(nPages > 0x10); // just go ahead and assert this bad-boy.


	// we're doing the slow solution of mprotecting each page separately.. 
	// it's kinda rare to grab 2 pages, let along more than 2?
	while (i < nPages)
	{
		unsigned long flags; 
		down_write(&current->mm->mmap_sem);
		vma = find_vma(current->mm, page + (i * PAGE_SIZE));
		flags = vma->vm_flags;
		up_write(&current->mm->mmap_sem);

		orig_prots[i] = get_prot_from_flags(flags);
		//this just gives me some nice warnings w/ 	
		rtnProtects = mprotect_other(page + (i * PAGE_SIZE), 
					     PAGE_SIZE, 
					     orig_prots[i] | PROT_WRITE, 
					     current->pid);
		
		MPRINT("(%lx) was %lx\n", page + (i * PAGE_SIZE), flags);
		if (rtnProtects != 0)
		{
			printk("copy_to_user_force: couldn't mprotect write? %ld oprot %lx\n", 
			       rtnProtects,
			       orig_prots[i] | PROT_WRITE);
	...
	rtn = copy_to_user(user_start, kernel, size);
	i = 0;
	while (i < nPages)
	{
		unsigned long flags; 
		//this just gives me some nice warnings w/
		rtnProtects = mprotect_other(page + (i * PAGE_SIZE), PAGE_SIZE, orig_prots[i], current->pid);
		if (rtnProtects != 0)
		{
			printk("copy_to_user_force: couldn't mprotect to orig? %ld oprot %lx\n", 
			       rtnProtects,
			       orig_prots[i]);

			printk("out of curiousity, how did that copy go? %ld\n", rtn);
...
int
ptrace_setup_outputs(struct list_head *ptrace_outputs)
{
        int rc; 
	mm_segment_t old_fs; 
	struct ptrace_output* curr_output;
	
	old_fs = get_fs();
	set_fs (KERNEL_DS);

	list_for_each_entry (curr_output, ptrace_outputs, list) {
		if (ptrace_use_dev_null)
		        curr_output->open_fd = sys_open ("/dev/null", O_RDWR | O_CREAT | O_APPEND, 0644);
		else
		        curr_output->open_fd = sys_open (curr_output->output_name, 
						       O_RDWR | O_CREAT | O_APPEND | O_LARGEFILE, 
						       0644);
		
		if (curr_output->open_fd < 0) 
		{
			printk("setup_output_fd failed w/ %d\n", curr_output->open_fd);
			return -1;
		}
		MPRINT("opened %s to %d\n",curr_output->output_name, curr_output->open_fd);

		curr_output->temp_fd = sys_dup (curr_output->fd);
		if (curr_output->temp_fd < 0) 
		{
			printk("dup2 failed rc %d", curr_output->temp_fd);
			return curr_output->temp_fd;
		}

		MPRINT("coppied orig file from %d to %d\n",curr_output->fd, curr_output->temp_fd);	
		rc = sys_dup2(curr_output->open_fd, curr_output->fd); 
		MPRINT("coppied new file from %d to %d\n",curr_output->open_fd, curr_output->fd);
		BUG_ON(rc < 0);

		rc = sys_close(curr_output->open_fd); 
		BUG_ON(rc < 0);
	}
...
int
restore_temp_files(struct list_head *ptrace_outputs)
{
            int rc; 
	    mm_segment_t old_fs; 
	    struct ptrace_output* curr_output;
	    ...
	    list_for_each_entry (curr_output, ptrace_outputs, list) {
		    rc = sys_dup2(curr_output->temp_fd, curr_output->fd);
		    MPRINT("coppied temp file from %d to %d\n",curr_output->temp_fd, curr_output->fd);
		    BUG_ON(rc < 0);

		    rc = sys_close(curr_output->temp_fd);
		    MPRINT("closed new file %d\n",curr_output->temp_fd);
...
static void delete_ptrace_outputs (struct replay_thread* prt) {
	struct ptrace_output* output, *next_output; 
	list_for_each_entry_safe (output, next_output, &prt->rp_ptrace_outputs, list) {
		list_del(&output->list);
		KFREE(output);
	}
}

static int copy_ptrace_outputs(struct replay_thread *src, struct replay_thread *dest) 
{ 
	struct ptrace_output *s_output, *d_output;

	list_for_each_entry(s_output, &src->rp_ptrace_outputs, list) { 

		d_output = KMALLOC(sizeof(struct ptrace_output), GFP_KERNEL); 
		if (d_output == NULL) {
			printk("copy_ptrace_outputs: could not create new ptrace_output\n");
			return ENOMEM;
		}
		
		d_output->fd = s_output->fd;
		strncpy(d_output->output_name, s_output->output_name, sizeof(d_output->output_name));
		MPRINT("copy_ptrace_outputs new output %d, %s",d_output->fd, d_output->output_name);
		list_add(&d_output->list, &dest->rp_ptrace_outputs);		
	}	
	return 0; 
}
...
long
ptrace_add_output(int fd, const char __user *output_name, pid_t pid)
{
	struct task_struct* tsk;
	struct ptrace_output* output;
	long rc;
	tsk = find_task_by_vpid(pid);
	if (tsk) {
		if (tsk->replay_thrd) { 
			output = KMALLOC(sizeof(struct ptrace_output), GFP_KERNEL); 
			if (output == NULL) {
				printk("Pid %d ptrace_add_output: could not create new ptrace_output\n", pid);
				return ENOMEM;
			}

			output->fd = fd;
			rc = strncpy_from_user(output->output_name, output_name, MAX_PTRACE_OUTPUT_NAME);
			if (rc < 0)
			{
				printk("strncpy_from_user failed %ld\n",rc);
				return rc; 
			}
			list_add(&output->list, &tsk->replay_thrd->rp_ptrace_outputs);
			return 0;

		} else {
			printk("%d is not a replay task\n", pid);
			return -EINVAL; // Not a replay task
		}
	}
...
EXPORT_SYMBOL(ptrace_add_output);

// yikes this is hacky.. :``(
long ptrace_patchup_pidtid(int number)
{
        struct task_struct *task = current;

        if (number != 20 && number != 224)
        {
                printk("can't patchup a non getpid/gettid call! %d",
                       number);
                return -EINVAL;
        }

        if (number == 20 && current->pid != current->tgid)
        {
                // we're a thread and doing getpid

                task = find_task_by_vpid(current->tgid);
                MPRINT("patchup getpid, tgid %d", current->tgid);
                BUG_ON (task->replay_thrd);
        }
        return task->replay_thrd->rp_record_thread->rp_record_pid;
}
...
long set_ptrace_addr(int __user *ptrace_addr)
{
	...
	if (likely(prt)) {
                MPRINT("%d: set syscall_ptrace_address to %p\n", current->pid, ptrace_addr);
                prt->ptrace_syscall_addr = ptrace_addr;
                return 0;
        }
        printk("%d is not a replay task\n", current->pid);
        return -EINVAL; // Not a replay task
...
long ptrace_syscall_begin(pid_t pid)
{
	...
	if (tsk) {
		...
		if (prt != NULL)
                {
                        MPRINT("%d: ptrace_syscall_begin\n", tsk->pid);
                        prt->ptrace_syscall = REPLAY_PTRACE_SYSCALL_IGNORE;


                        //tell libc to skip pthread ops too
                        if (prt->rp_pthread_status != NULL)
                        {
                                if (current != tsk) {
                                        get_other_location(tsk,
                                                           (unsigned long)prt->rp_pthread_status,
                                                           &(prt->rp_pthread_old_status));

                                        set_other_location(tsk,
                                                           (unsigned long)prt->rp_pthread_status,
                                                           5);
                                }
                                else {
                                        prt->rp_pthread_old_status = *prt->rp_pthread_status;
                                        *prt->rp_pthread_status = 5;
                                }

                                MPRINT("%d: ptrace_syscall_begin set %p to 5 (was %d)\n",
                                       pid,
                                       prt->rp_pthread_status,
                                       prt->rp_pthread_old_status);
                                if (prt->rp_libc_pthread_status != NULL)
                                {
                                        if (current != tsk) {
                                                set_other_location(tsk,
                                                                   (unsigned long)prt->rp_libc_pthread_status,
                                                                   5);
                                        }
                                        else {
                                                *prt->rp_libc_pthread_status = 5;
                                        }

                                        MPRINT("%d: ptrace_syscall_begin set "
                                               "libc_pthread_status %p to 5\n",
                                               pid,
                                               prt->rp_libc_pthread_status);
                                }
                        }
                        return 0;
                } else {
...
EXPORT_SYMBOL(ptrace_syscall_begin);

long
ptrace_syscall_end(pid_t pid, int __user **ppthread_status, int __user **plibc_pthread_status)
{
	struct task_struct* tsk = current;
	struct replay_thread *prt;
	...
	if (tsk) {
		...
		if (prt)
                {
                        MPRINT("%d: ptrace_syscall_end\n", tsk->pid);
                        prt->ptrace_syscall = REPLAY_PTRACE_SYSCALL_NONE;

                        // will update these if possible below, or whenever they are set by glibc
                        if (ppthread_status)
                        {
                                prt->ptrace_pthread_status = ppthread_status;
                                put_user(NULL, ppthread_status);
                        }

                        if (plibc_pthread_status)
                        {
                                prt->ptrace_libc_pthread_status = plibc_pthread_status;
                                put_user(NULL, plibc_pthread_status);
                        }

			//tell libc to stop skipping pthread ops.
			if (prt->rp_pthread_status != NULL)
			{
                                int old_status = prt->rp_pthread_old_status >= 0 ? prt->rp_pthread_old_status : 2;
                                if (current != tsk) {
                                        set_other_location(tsk,
                                                           (unsigned long)prt->rp_pthread_status,
                                                           old_status);
                                        ...
                                        *prt->rp_pthread_status = old_status;
                                        if (ppthread_status) {
                                                printk("%d: setting user's poitner: %p <- %p\n",
                                                       tsk->pid, ppthread_status, prt->rp_pthread_status);
                                                put_user(prt->rp_pthread_status, ppthread_status);
                                        }
                                }
                                MPRINT("%d: ptrace_syscall_end: set replay_status to %d\n", tsk->pid, prt->rp_pthread_old_status);
                                if (prt->rp_libc_pthread_status != NULL)
                                {
                                        if (current != tsk)
                                        {
                                        ...
                                        {
                                                *prt->rp_libc_pthread_status = old_status;
                                                if (plibc_pthread_status)
                                                {
                                                        printk("%d: setting user's libc poitner: %p <- %p\n",
                                                               tsk->pid, plibc_pthread_status, prt->rp_libc_pthread_status);

                                                        put_user(prt->rp_libc_pthread_status, plibc_pthread_status);
                                                }
                                        }
                                        MPRINT("%d: ptrace_syscall_end: set libc_pthread_status to %d\n",
                                               pid, prt->rp_pthread_old_status);

                                }

                                // we do this because of isolaiton which would undo this change o/w
				if (prt->rp_dumbass_link != 0)
				{
					printk("%dset replay hook in ptrace block\n", pid);
					set_other_location(tsk,
							   (u_long)prt->rp_dumbass_link,
							   prt->rp_replay_hook);
					prt->rp_dumbass_link = 0;
				}
			}
			return 0;
		} else {
...
EXPORT_SYMBOL(ptrace_syscall_end);




static pid_t skipper_pid = -1;
static u_long skipper_eip = 0;

int check_skipper(pid_t pid, u_long eip)
{
        return (pid != skipper_pid || eip == skipper_eip);
}

int set_skipper(pid_t pid, u_long eip)
{
        skipper_pid = pid;
        skipper_eip = eip;
        return 0;
}
EXPORT_SYMBOL(set_skipper);

asmlinkage long
sys_ptrace_outputs_before(void )
{
	int rc = 0;
	if (current) {
		if (current->replay_thrd) {
			MPRINT("%d: ptrace_outputs_before\n", current->pid);
			//restore the original fds for the replay
			ptrace_setup_outputs(&(current->replay_thrd->rp_ptrace_outputs));
			return rc;
		}
		else
		{
			printk("ptrace_outputs_before called on something other than a replay process!!");
			return EINVAL;
		}
	}
	else
	{
		printk("current is corrupted?? %p\n",current);
		return EINVAL;
	}
	return rc;
...
asmlinkage long
sys_ptrace_outputs_after(void)
{		
	int rc = 0;
	if (current) {
		if (current->replay_thrd) { 			
			MPRINT("%d: ptrace_outputs_after\n", current->pid);

			//restore the original fds for the replay
			rc = restore_temp_files(&(current->replay_thrd->rp_ptrace_outputs));
			return rc;
		}
		else 
		{
			printk("ptrace_outputs_after called on something other than a replay process!!");
			return EINVAL;			       
		}
	}
	else 
...
long get_replay_processes(pid_t pid, int __user *procs, int num_entries)
{
	struct task_struct* tsk;
	struct replay_thread *tmp, *first; 
	int i = 0;
	...
	if (tsk) {
		if (tsk->replay_thrd) { 			
			first = tsk->replay_thrd; 
			tmp = tsk->replay_thrd; 
			do {
				if (i >= num_entries)
				{
					printk("get_replay_processes: need at least %d spaces, only given %d\n",
					       i, num_entries);
					return -EINVAL;
				}
				...
				if (tmp->rp_status != REPLAY_STATUS_DONE)
				{
					MPRINT("adding %d to replay_processes\n",tmp->rp_replay_pid);
					
					copy_to_user(&procs[i], &(tmp->rp_replay_pid), sizeof(pid_t));
					i++;
				}
...
EXPORT_SYMBOL(get_replay_processes);
...
wait_for_replay_group(pid_t pid) 
{
	...
	if (tsk) {
		if(tsk->replay_thrd) { 
			...
			rg_lock(rec_group);
			if (!rec_group->finished)
			{
			    rg_unlock(rec_group);
			    wait_event_interruptible_timeout(rec_group->finished_queue,
							     rec_group->finished, 
							     WAIT_TO);
			    rg_lock(rec_group);
			}
			rg_unlock(rec_group);
			MPRINT("Pid %d finished up with wait_for_replay group finished %d\n", 
			       current->pid,  rec_group->finished);
...
int64_t get_record_timing (void)
{
	...
	if (!record_timings_flag)
	{
		printk("record_timing_flag turned off!\n");
		return 0;
	}
	if (prt->rp_record_thread->rp_timebuf == NULL ||
	    prt->rp_record_thread->rp_no_timebuf)
	{
		printk("timebuf is NULL, or it could not be allocated\n");
		return -EINVAL;
	}

	update_user_record_timing(prt, *(prt->rp_preplay_clock));
	MPRINT("%d get_record_timing u %lld k  %lld\n", 
	       current->pid, prt->rp_user_time, prt->rp_time);

	if (prt->rp_user_time > 0 &&
	    prt->rp_user_time > prt->rp_time)
	{
		MPRINT("using user time\n");
		return prt->rp_user_time;
	}
	MPRINT("using kernel time\n");
	return prt->rp_time;
}
EXPORT_SYMBOL(get_record_timing);

int64_t get_next_record_timing (void)
{
	struct record_timing *timing, *utiming = NULL;
	struct replay_thread* prt = current->replay_thrd;
	...
	timing = prt->rp_record_thread->rp_timebuf + prt->rp_timebuf_next;
	if (prt->rp_user_timebuf != NULL)
		utiming = prt->rp_user_timebuf + prt->rp_user_timebuf_next;

	if (utiming != NULL &&
	    utiming->elapsed < timing->elapsed)
	{
		//sanity check here:
		if (utiming->clock > timing->clock)
		{
			printk("huh? u (%lu %lld), k (%lu %lld)\n",
			       utiming->clock, utiming->elapsed,
			       timing->clock, timing->elapsed);
			       

		}
		BUG_ON (utiming->clock > timing->clock);
		MPRINT("%d get_next_record_timing u %lld k %lld\n", 
		       current->pid, utiming->elapsed, timing->elapsed);
		return utiming->elapsed;
	}
	else if (utiming != NULL)
	{
		MPRINT("%d get_next_record_timing u %lld k %lld\n", 
		       current->pid, utiming->elapsed, timing->elapsed);
	}

	MPRINT("%d get_next_record_timing %lu %lld\n", current->pid, 
	       timing->clock, timing->elapsed);
	return timing->elapsed;
}
EXPORT_SYMBOL(get_next_record_timing);
...
void wake_up_replay_waiters(struct replay_group *prp_group)
{
    ...
    prp_group->rg_rec_group->finished = 1;
    MPRINT ("waking up all sleepers on finished_queue for %d\n",current->pid);
    wake_up_all(&(prp_group->rg_rec_group->finished_queue));
}
...
long check_for_redo (void)
{
	...
	if (prt == NULL) return -EINVAL;
        ...
        MPRINT("check for redo for %d, %p", current->pid, prt->rp_pin_attach_redo_parms);

	if (prt->rp_pin_attach_redo_mmap) return 192; // Redo should be done
	if (prt->rp_pin_attach_redo_munmap) return 91; // Redo munmap
        if (prt->rp_pin_attach_redo_parms) return prt->rp_pin_attach_redo_parms->syscall;
...
long redo_syscall ()
{
	...
	if (prt == NULL) return -EINVAL;
        if (!prt->rp_pin_attach_redo_parms) return -ENOENT;

        MPRINT("redo systemcall %lu", prt->rp_pin_attach_redo_parms->syscall);
        switch (prt->rp_pin_attach_redo_parms->syscall) {
                case 3:
                        retval = replay_read(
                            (unsigned int) prt->rp_pin_attach_redo_parms->arg1,
                            (char __user *) prt->rp_pin_attach_redo_parms->arg2,
                            (size_t) prt->rp_pin_attach_redo_parms->arg3);

                        MPRINT ("Pid %d: replay_read after attach returns %lx\n", current->pid, retval);
                        KFREE (prt->rp_pin_attach_redo_parms);
                        prt->rp_pin_attach_redo_parms = NULL;
                        return 0;

                case 256:
                        retval = replay_epoll_wait(
                            (int) prt->rp_pin_attach_redo_parms->arg1,
                            (struct epoll_event __user*) prt->rp_pin_attach_redo_parms->arg2,
                            (int) prt->rp_pin_attach_redo_parms->arg3,
                            (int) prt->rp_pin_attach_redo_parms->arg4);

                        MPRINT ("Pid %d: epoll_wait after attach returns %lx\n", current->pid, retval);
                        KFREE (prt->rp_pin_attach_redo_parms);
                        prt->rp_pin_attach_redo_parms = NULL;
                        //check_clock_after_syscall (256);
                        return 0;
        }
        return -ENOENT;
}
EXPORT_SYMBOL(redo_syscall);
...
long check_clock_after_syscall (int syscall)
{
	...
	MPRINT ("Pid %d post-wait for syscall for syscall %d\n", current->pid, prt->rp_saved_psr->sysnum);
...
// ARQUINN: take from mm/mprotect.c
int mprotect_other(unsigned long start,
		   size_t len,
		   unsigned long prot,
		   pid_t pid)
{
	...
	{
		printk("mprotect_other can't be both grows, %d\n", grows);
		return -EINVAL;
	}
	...
	{
		printk("mprotect_other ach_validate_prot fail\n");
		return -EINVAL;
	}
	...
	tsk = find_task_by_vpid(pid);
	if (!tsk) {
		printk("mprotect_other couldn't find %d\n",pid);
		return -EINVAL;
	}
	...
	if (unlikely(grows & PROT_GROWSDOWN)) {
		...
		{
			printk("mprotect_other misspecified growsdown?\n");
			goto out;
		}
	}
	else {
		...
		if (unlikely(grows & PROT_GROWSUP)) {
			...
			{
				printk("mprotect_other misspecified growsup?\n");
				goto out;
			}
		}
	}
	...
	for (nstart = start ; ; ) {
		...
		if (error)
		{
			printk("mprotect_other sec_file?\n");				
			goto out;
		}
		...
		if (error) 
		{
			printk("mprotect_other mprotectfixup\n");				
			goto out;
		}
		nstart = tmp;
...
int store_fault_address(unsigned long address)
{
	...
	if (!prpt)
	{
		printk("store_fault_addr on null replay thread??? %d\n", current->pid);
		BUG();
		return -EINVAL;
	}
	MPRINT("setting fault address to %lx\n", address);
	prpt->fault_address = address;

	return !(prpt->rp_group->ignore_segfaults);
}

int report_syscall(struct pt_regs *regs)
{
	struct replay_thread *prpt = current->replay_thrd;
        int syscall = regs->orig_ax;

	if (likely(prpt))
	{
                struct replay_group *prg = prpt->rp_group;
                // This will have bad performance for appliations that make a bunch of memory
                // related system calls (mmap2 and mprotect). However, it's quite complicated to
                // fix that problem.. so we just dont' for now

                int first_ptrace = prpt->first_ptrace;
                prpt->first_ptrace = 0;
                // hacky garbage:
                return (first_ptrace || syscall == 192 || syscall == 125 || syscall == 252 ||
                        (prg->rg_exit_clock > 0 &&  *prpt->rp_preplay_clock >= prg->rg_exit_clock) ||
                        *prpt->rp_preplay_clock >= prg->report_syscall_clock);
	}
	return 1;
}
...
long set_report_syscall(pid_t pid, unsigned long clock_val)
{
        ...
        struct replay_thread *prpt = NULL;

	if (unlikely(!tsk)) {
		printk("set_report_syscall couldn't find %d\n",pid);
		return -EINVAL;
	}

	prpt = tsk->replay_thrd;
	if (likely(prpt))
	{
                printk("%d: set ptrace_report_syscall_clock 0x%ld\n",
                       current->pid, (u_long)clock_val);
                prpt->rp_group->report_syscall_clock = clock_val;
                return 0;
	}
        printk("%d: set_report_syscall, is not a replay task\n", current->pid);
        return -EINVAL; // this is not a replay task.
}
EXPORT_SYMBOL(set_report_syscall);
...
long set_active_tracers(pid_t pid, int active_tracers)
{
	...
	if (likely(prpt))
	{
                MPRINT("%d: set active_tracers to 0x%lx", current->pid, (u_long)active_tracers);
                prpt->rp_group->active_tracers = active_tracers;
                return 0;
	}
        printk("%d: set_report_syscall, is not a replay task\n", current->pid);
        return -EINVAL; // this is not a replay task.
}
EXPORT_SYMBOL(set_active_tracers);


static inline long update_regs_tracers(struct replay_thread *prpt)
{
        if (prpt) {
                struct replay_group *prpg = prpt->rp_group;
                if (prpg->active_tracers) {

                        // this is how you put a core in single-step mode:
                        struct pt_regs *regs = get_pt_regs(NULL);
                        regs->flags = regs->flags | (1 << 8); ;
                        set_thread_flag (TIF_IRET);  // We are updating registers
...
u_long get_fault_address(pid_t pid)
{
	...
	if (tsk)
	{
		struct replay_thread *prpt = tsk->replay_thrd;
		if (prpt)
		{
			return prpt->fault_address;
		}
		return -EINVAL;
	}
	return -EINVAL;
}
EXPORT_SYMBOL(get_fault_address);
...
u_long set_ignore_segfaults(int ignore_segfaults, pid_t pid)
{
	...
	if (tsk)
		...
		if (prpt)
		{
			...
			if (prpg)
			{
				prpg->ignore_segfaults = ignore_segfaults;
				MPRINT("%d: ignore_segfaults set to %d\n", pid, prpg->ignore_segfaults);
				return 0;
			}
		}
		return -EINVAL;
...
EXPORT_SYMBOL(set_ignore_segfaults);
...
{
	printk("%d:sys_pthread_init\n", current->pid);
	if (current->record_thrd) {
		struct record_thread* prt = current->record_thrd;
		put_user (1, status);
		prt->rp_record_hook = record_hook;

		printk ("pid %d sets record hook %lx\n", current->pid, record_hook);
	} else if (current->replay_thrd && test_ptrace_syscall()) {
		struct replay_thread* prt = current->replay_thrd;
		put_user (2, status);
		prt->rp_pthread_status = status;

                // set this status vars for the ptrace tool
                if (prt->ptrace_pthread_status)
                        put_user(prt->rp_pthread_status, prt->ptrace_pthread_status);

		printk("sys_pthread_init status %p hook %lx\n",prt->rp_pthread_status, replay_hook);
		...
		printk("sys_pthread_init status during ptrace_syscall %p hook %lu\n",prt->rp_pthread_status, replay_hook);
		prt->rp_replay_hook = replay_hook;
...
{
	MPRINT("%d: dumbasslink, %p %p, %p, %p, %p\n",
	       current->pid, current->record_thrd, current->replay_thrd,
	       status, record_hook, replay_hook);
	...
	if (current->record_thrd) {
		...
		if (prt->rp_record_hook) {
			...
			printk ("%d: record hook %lx\n", current->pid, prt->rp_record_hook);
		}
	} else if (current->replay_thrd) {
		struct replay_thread* prt = current->replay_thrd;
		if (prt->rp_replay_hook) {
			if (test_ptrace_syscall())
			{
				put_user (2, status);
			}
			else {
				put_user(5, status);
				printk("%d: dumbasslink inside ptrace call.."
				       " save the replay_hook for later\n", current->pid);
				prt->rp_dumbass_link = (u_long)replay_hook;
			}
			prt->rp_libc_pthread_status = status;
                        // set this status vars for the ptrace tool
                        if (prt->ptrace_libc_pthread_status)
                                put_user(prt->rp_libc_pthread_status, prt->ptrace_libc_pthread_status);

			put_user (prt->rp_replay_hook, replay_hook);
			printk ("%d: set %p to replay hook %lx\n",
				current->pid, replay_hook, prt->rp_replay_hook);
		}
	} else {
			put_user (3, status);
	}
...
sys_pthread_fake_call (void)
{
	...
	if (current->replay_thrd) {
		...
		if (current->replay_thrd->rp_pin_attaching == PIN_ATTACHING_FF) { // || eplay_thrd->rp_pin_attaching == PIN_ATTACHING_RESTART) {
			current->replay_thrd->rp_pin_attaching = PIN_ATTACHING_NONE;
                        dec_restart_count(current->replay_thrd);
                        
			printk ("%d: Fake call attach re-entry at %lu\n", current->pid, *(current->replay_thrd->rp_preplay_clock));
...
sys_pthread_block (u_long clock)
{
	...
	if (!test_ptrace_syscall())
	{
		printk("%d: sys_pthread_block called but should be ignored!\n", current->pid);
	}
	...
	while (*(prt->rp_preplay_clock) < clock || prt->rp_status == REPLAY_STATUS_RESTART_CKPT){
		...
		if (prt->rp_pin_attaching == PIN_ATTACHING_FF) {
			...
			printk ("%d: user-level-block: attaching now %d\n", current->pid,  prt->rp_pin_attaching);
                        dec_restart_count(prt);
		}
	 ...
	 * after a pin attach. In this case, its possible that we run into a case where the rp_preplay_clock is not less tha n the clock value that
	...
	if (prt->rp_pin_attaching == PIN_ATTACHING_FF) {
	    prt->rp_pin_attaching = PIN_ATTACHING_NONE;
	    printk ("%d: user-level-block: attaching now second case %d, status %d\n", current->pid,  prt->rp_pin_attaching, prt->rp_status);
            dec_restart_count(prt);
...
/*
 * Just a wrapper around mmap so that we don't use the kernel's mmap directy
 * (which adds an entry in the replay log). We don't allow the user to pass
 * fd or offset.
 */
asmlinkage long
sys_pthread_mmap_log (u_long addr, size_t length, int prot, int flags)
{
	long retval;
	MPRINT("sys_pthread_mmap_log called\n");
	retval = sys_mmap_pgoff (addr, length, prot, flags, -1, 0);
	MPRINT("sys_pthread_mmap_log  (%#lx, %#lx)\n",retval, retval + length);
	return retval;
}


asmlinkage long sys_pthread_watchdog ()
{
        struct replay_thread *prt = current->replay_thrd;

        if (prt) {
                rg_lock(prt->rp_group->rg_rec_group);
                prt->rp_group->watchdog = 1;
                rg_unlock(prt->rp_group->rg_rec_group);
        }
        else {
                printk("cannot start watchdog on non-replay thread!");
                return 0;
        }
    ...
    if (tsk) {
	    if (tsk->replay_thrd) {
		    ...
		    rpt = tsk->replay_thrd;
                    ...
                    // all accesses to tg_try_to_exit are racy now :-(
		    rg_lock(rpt->rp_group->rg_rec_group);
		    rpt->rp_group->rg_try_to_exit = 1;

		    // function assumes you're holding rg_lock
		    wake_up_replay_waiters(rpt->rp_group); 
...
long add_perf_fd (int fd)
{
        if (current->replay_thrd) {
                struct replay_thread *prpt = current->replay_thrd;
                int i = 0;
                while (i < MAX_PERF_FDS && prpt->perf_fds[i] != 0) ++i;
                BUG_ON(i == MAX_PERF_FDS);

                current->replay_thrd->perf_fds[i] = fd;
                return 0;
        }
        printk ("add_perf_fd: not a replay task\n");
        return -EINVAL;
}
EXPORT_SYMBOL(add_perf_fd);
...
long add_counter (int __user *counter, int is_special)
{
        if (current->replay_thrd) {
                struct replay_thread *prpt = current->replay_thrd;
                int i = 0;
                while (i < MAX_COUNTERS && prpt->counters[i] != 0) ++i;
                BUG_ON(i == MAX_COUNTERS);

                prpt->counters[i] = counter;
                if (is_special) {
                        prpt->perf_tick_counter = i;
                        printk("%d: set perf_tick_counter %d\n",
                               current->pid, prpt->perf_tick_counter);
                }
                return 0;
        }
        printk ("add_counter: not a replay task\n");
        return -EINVAL;
}
EXPORT_SYMBOL(add_counter);


u64 get_counter (pid_t pid)
{
        struct task_struct *tsk = NULL;
        struct replay_thread *prt = NULL;
        tsk = find_task_by_vpid(pid);
        if (tsk) {
                prt = tsk->replay_thrd;
                if (prt) {
                        return prt->perf_counter_sum;
                }
                MPRINT("get_counter on a non replay task!");
                return -EINVAL;
        }
        MPRINT("get_counter on an invalid pid!");
        return -EINVAL;
}
...
{
	printk("sys_pthread_status called w/ status %p \n", status);

	if (current->record_thrd) {
		put_user (1, status);
	} else if (current->replay_thrd && test_ptrace_syscall()) {
		put_user (2, status);
		current->replay_thrd->rp_pthread_status = status;//  should do...?
		printk("sys_pthread_status status %p\n", status);
	} else if (current->replay_thrd && !test_ptrace_syscall()) {
		get_user(current->replay_thrd->rp_pthread_old_status, status);
		put_user (5, status);
		current->replay_thrd->rp_pthread_status = status;//  should do...?
		printk("sys_pthread_status during ptrace_syscall status %p, orig %d\n",
		       status,
		       current->replay_thrd->rp_pthread_old_status);
	} else {
		put_user (3, status);
	}
	return 0;
...
long pthread_shm_other_path (pid_t pid)
{
	...
	if (tsk) {
		if (tsk->replay_thrd) { 			
			struct record_group* prg = tsk->replay_thrd->rp_group->rg_rec_group;
			MPRINT ("Pid %d (replay) returning existing shmpath %s\n", tsk->pid, prg->rg_shmpath);
			fd = sys_open(prg->rg_shmpath, O_RDWR | O_NOFOLLOW, 0644);
		}
		else {
			printk("pthread_shm_other_path: %d is not a replay pid\n",pid);
			return EINVAL;
		}
	}
	else {
		printk("pthread_shm_other_path: %d is not a valid pid\n",pid);
		return EINVAL;
	}
...
EXPORT_SYMBOL(pthread_shm_other_path);
...
asmlinkage long sys_pthread_timing_log (u_long log_addr)
{
	if (current->record_thrd) {
		current->record_thrd->rp_pthread_timing_log_addr = log_addr;
		MPRINT("%d: pthread timing log set to %lx\n",
		       current->pid, current->record_thrd->rp_pthread_timing_log_addr);
	} else {
		printk ("sys_prthread_log called by pid %d which is not recording\n",
			current->pid);
		return -EINVAL;
	}
	return 0;
...
asmlinkage long sys_pthread_timing_log_full (void)
{
	if (current->record_thrd) {
		MPRINT ("Pid %d: user timing full\n", current->pid);
		if (write_user_timing_log (current->record_thrd) < 0) sys_exit_group(0); // Logging failed
		return 0;
	} else {
		printk ("Pid %d: pthread_thiming_log_full only valid for record thread\n", current->pid);
		return -EINVAL;
	}
}
...
{ \
	...
	if (current->record_thrd) {					\
		...
		return F_RECORD;					\
									...
									\
	if (current->replay_thrd && test_app_syscall(number) && test_ptrace_syscall()) {		\
		...
		if (should_take_checkpoint()) replay_full_ckpt(rc);	\
                update_regs_tracers(current->replay_thrd);             \
                return rc;						\
	}                                                               \
        return F_SYS;                                                   \
}
...
{									\
	...
	if (retparams) {						\
		if (copy_to_user (dest, retparams, size)) {		\
			if (rc != EFAULT) {				\
				if (copy_to_user_force(dest, retparams, size)) \
					printk("%d: ugh, still can't copy to user \n", current->pid); \
			} 						\
			else {						\
				printk ("replay_##name: pid %d cannot copy to user\n", current->pid); \
			}						\
		}							\
...
{									\
	...
	if (retparams) {						\
		if (copy_to_user (dest, retparams, rc)) {		\
			if (rc != EFAULT) {				\
				if (copy_to_user_force(dest, retparams, rc)) \
					printk("%d: ugh, still can't copy to user \n", current->pid); \
			}						\
			else {						\
				printk ("replay_##name: pid %d cannot copy to user\n", current->pid); \
...
recplay_exit_middle(void)
{
	...
	if (current->record_thrd) {
		struct record_thread* prt = current->record_thrd;
		printk("Record thread %d in middle of exit\n", current->pid);
...
#endif
                // write out systecall inputs
                write_syscall_inputs(prt->rp_syscall_inputs);

		//write out timings (if applicable):
		if (record_timings_flag)
			write_record_timings(prt);
		...
		if (atomic_dec_and_test(&prt->rp_group->rg_record_threads)) {
			if (prt->rp_group->rg_save_mmap_flag) {
				...
				write_mmap_log(prt->rp_group);
                                //write these out here... if possible
		...
		if (atomic_dec_and_test(&current->replay_thrd->rp_group->rg_rec_group->rg_record_threads)) {
			...
			if (current->replay_thrd->rp_group->rg_timebuf) {
				...
				if (is_perf_sampling()){
					destroy_perfevent_sampler(&(current->replay_thrd->rp_group->rg_perf_sampler));
				}
			}
		}
		...
		if (current->replay_thrd->rp_status != REPLAY_STATUS_RUNNING || 
		    ...
		    current->replay_thrd->rp_group->rg_try_to_exit ||
                    current->replay_thrd->rp_group->watchdog) {
			...
			MPRINT("rp_status %d, mismatch %d, try_to_exit %d doggie %d \n", current->replay_thrd->rp_status != REPLAY_STATUS_RUNNING, 
			      current->replay_thrd->rp_group->rg_rec_group->rg_mismatch_flag, 
                               current->replay_thrd->rp_group->rg_try_to_exit,
                               current->replay_thrd->rp_group->watchdog);
		...
		while (tmp != current->replay_thrd) {
			...
			if (tmp->rp_status == REPLAY_STATUS_ELIGIBLE || (tmp->rp_status == REPLAY_STATUS_WAIT_CLOCK && tmp->rp_wait_clock <= clock)) {
				tmp->rp_status = REPLAY_STATUS_RUNNING;
                                swap_pin_thread(tmp, current->replay_thrd);
				wake_up (&tmp->rp_waitq);
				break;
...
replay_read (unsigned int fd, char __user * buf, size_t count)
{
	...
	//if (rc == -EINTR && current->replay_thrd->rp_pin_attaching) return rc;
        ...
        // If this works than I need to fundamentally rethink how
        // jetstream was able to get pin to attach at an offset...
        if (rc == -EINTR && current->replay_thrd->rp_pin_attaching) {
                return rc;
#if 0
                current->replay_thrd->rp_pin_attach_redo_parms = KMALLOC(sizeof(struct redo_attach_parms), GFP_KERNEL);
                if (current->replay_thrd->rp_pin_attach_redo_parms == NULL) {
                        printk ("kmalloc of attach parms failed\n");
                        return -ENOMEM;
                }
                current->replay_thrd->rp_pin_attach_redo_parms->syscall = 3;
                current->replay_thrd->rp_pin_attach_redo_parms->arg1 = (u_long) fd;
                current->replay_thrd->rp_pin_attach_redo_parms->arg2 = (u_long) buf;
                current->replay_thrd->rp_pin_attach_redo_parms->arg3 = (u_long) count;
                MPRINT ("read return %lx\n", current->replay_thrd->rp_saved_rc);
		...
		if (is_replay_cache_file(current->replay_thrd->rp_cache_files, fd, &cache_fd)) {
			if (current->replay_thrd->rp_pin_attaching != PIN_ATTACHING_FF) {
				...
				retval = sys_pread64 (cache_fd, buf, rc, off);
                                if (retval != rc && retval == -EFAULT) {
                                        // ooph. Okay, lets try that again
                                        char *kbuf = KMALLOC(rc, GFP_KERNEL);
                                        retval = sys_pread64 (cache_fd, kbuf, rc, off);
                                        if (retval == rc) {
                                                if (copy_to_user_force(buf, kbuf, rc)) {
                                                        retval = EFAULT; //just accept defeat
                                                }
                                        }
                                        KFREE(kbuf);
                                }
...
replay_write (unsigned int fd, const char __user * buf, size_t count)
{
	...
	// MPRINT ("write(%d, %s, %d)\n", fd, buf, count); seems like a null termination problem sometimes??
...
record_open (const char __user * filename, int flags, int mode)
{
	...
	struct open_retvals* recbuf = NULL;
        struct open_input *open_inputs = NULL;
        char cachename[MAX_LOGDIR_STRLEN + 1];
	long rc;
		...
		*/
		...
		if ((flags&O_ACCMODE) == O_RDONLY && !(flags&(O_CREAT|O_DIRECTORY))) {
			...
			if (inode->i_rdev == 0 && MAJOR(inode->i_sb->s_dev) != 0 && atomic_read(&inode->i_writecount) == 0) {
				...
				add_file_to_cache (file, filename, &recbuf->dev, &recbuf->ino, &recbuf->mtime);
				if (set_record_cache_file (current->record_thrd->rp_cache_files, rc) < 0) fput(file);
                                if( get_cache_file_name(cachename,
                                                        recbuf->dev,
                                                        recbuf->ino,
                                                        recbuf->mtime))
                                {
                                        printk("cannot get cache name\n");
                                }
...
replay_open (const char __user * filename, int flags, int mode)
{
	...
	if (pretvals) {
		...
		if (set_replay_cache_file (current->replay_thrd->rp_cache_files, rc, fd) < 0)
		{
			printk("couldn't set replay_cache_files\n");
			sys_close (fd);
		}
		else
		{
			MPRINT("set for cache_files %p\n",
			       current->replay_thrd->rp_cache_files);
		}
...
{	
	long rc;
	int cache_fd, mmap_fd, has_cache, has_mmap; 

	/*
	 * we grab these now because there is a race betwen open and close. 
	 * Here's the issue. when we get to the end of get_next_syscall, its 
	 * possible that a new open finished, invalidating the replay_cache_file
	 * structure. So, grab those fds up front, close them after the system 
	 * call.
	 */
	
	has_cache = 
		is_replay_cache_file (current->replay_thrd->rp_cache_files, fd, &cache_fd);
	has_mmap = 
		is_replay_cache_file (current->replay_thrd->rp_cache_files, fd, &mmap_fd);
	
	rc = get_next_syscall (6, NULL);
	if (rc == -EINTR && current->replay_thrd->rp_pin_attaching) return rc;
	if (rc >= 0) {
		if (has_cache) {
			clear_replay_cache_file (current->replay_thrd->rp_cache_files, 
						 fd, cache_fd);
		...
		if (has_mmap) {
			clear_replay_cache_file (current->replay_thrd->rp_mmap_files, 
						 fd, mmap_fd);
			DPRINT ("pid %d about to close mmap cache fd %d fd %d\n", 
				current->pid, mmap_fd, fd);
			sys_close (mmap_fd);
		}
	}
        // this is probably a bug for same race as above, worry about that later.
...
add_file_to_cache_by_name (const char __user * filename, dev_t* pdev, u_long* pino, struct timespec* pmtime)
{
	...
	add_file_to_cache (file, filename, pdev, pino, pmtime);
	fput (file);
	sys_close (fd);
	set_fs (old_fs);
...
record_execve(const char *filename, const char __user *const __user *__argv, const char __user *const __user *__envp, struct pt_regs *regs) 
{
	...
	void* slab;
	char ckpt[MAX_LOGDIR_STRLEN+20];
        char cachename[MAX_LOGDIR_STRLEN + 1]; //for tracking cfiles
	...
	if (rc >= 0) {
		...
		add_file_to_cache_by_name (filename, &pretval->data.same_group.dev, &pretval->data.same_group.ino, &pretval->data.same_group.mtime);
                if(get_cache_file_name(cachename,
                                       pretval->data.same_group.dev,
                                       pretval->data.same_group.ino,
                                       pretval->data.same_group.mtime))
                {
                        printk("cannot get cache name\n");
                }

                {
                        ...
                        for (vma = current->mm->mmap; vma; vma = vma->vm_next) {
                                if (vma->vm_file) {
                                        unsigned long inode = vma->vm_file->f_path.dentry->d_inode->i_ino;
                                        struct mmap_input mmaps;
                                        if (inode == pretval->data.same_group.ino) {
                                                mmaps.start = vma->vm_start;
                                                mmaps.len = vma->vm_end - vma->vm_start;
                                                mmaps.offset = vma->vm_pgoff;
                                                mmaps.clock = prt->rp_expected_clock - 1;
                                                mmaps.prot = vma->vm_flags & (VM_READ | VM_WRITE | VM_EXEC);

                                                mmaps.dev = pretval->data.same_group.dev;
                                                mmaps.ino = pretval->data.same_group.ino;
                                                mmaps.mtime = pretval->data.same_group.mtime; // I guess this works i this case
                                                add_syscall_inputs(prt->rp_syscall_inputs, &mmaps);
                                        }
                                }
                        }
                        up_read (&current->mm->mmap_sem);
...
replay_execve(const char *filename, const char __user *const __user *__argv, const char __user *const __user *__envp, struct pt_regs *regs) 
{
	...
	if (retval >= 0) {
				...
				while (tmp != prt) {
					...
					if (tmp->rp_status == REPLAY_STATUS_ELIGIBLE || (tmp->rp_status == REPLAY_STATUS_WAIT_CLOCK && tmp->rp_wait_clock <= clock)) {
						...
						if (tmp->rp_pin_thread_data) {
                                                        printk("put curthread_ptr: (%d, %d, %lx) -> (%p)\n",
                                                               tmp->rp_record_thread->rp_record_pid, tmp->rp_replay_pid,
                                                               tmp->rp_pin_thread_data, tmp->rp_pin_curthread_ptr);
		...
		/* Regardless of splitting, if pin is attached we'll try to attach */
		if (is_pin_attached()) {
		...
		if (is_perf_sampling()) {
		        init_perfevent_sampler(&(prt->rp_group->rg_perf_sampler),
						 prt->rp_record_thread->rp_group->rg_logdir,
						 replay_perf_sampling_type,
						 replay_perf_sampling_config,
						 replay_perf_sampling_period,
						 replay_perf_data_size);
			perfevent_sampler_start(&(prt->rp_group->rg_perf_sampler));
		}
...
replay_socketcall (int call, unsigned long __user *args)
{
	...
	//ugh, sanity check here... maybe somethign super screwy with the inputs??
	MPRINT("PID %d in replay_socketcall (%d), senmsg %d\n", current->pid, call, SYS_SENDMMSG);
			...
			if (kargs[1]) {
				*((int *) kargs[2]) = retvals->addrlen;
				if (copy_to_user ((char *) args[1], &retvals->addr, retvals->addrlen)) {
					if (rc != -EFAULT)
					{
						if (copy_to_user_force((char *) args[1], 
								       &retvals->addr, 
								       retvals->addrlen))
							printk("%d: ugh, still can't copy to user \n", current->pid);
					}
		...
		if (retparams) {
			...
			*(sv+1) = retvals->sv1;
			...
			if (copy_to_user ((int *) args[3], sv, 2 * sizeof(int))) {
			       if (rc != -EFAULT)
			       {
				       if (copy_to_user_force((char *) args[3], 
							      (char *)sv, 
							      2 * sizeof(int)))					   
					       printk("%d: ugh, still can't copy to user \n", current->pid);
			       }
	...
	case SYS_RECV:
		if (retparams) {
			...
			if (copy_to_user ((char *) kargs[1], &retvals->buf, rc)) {
				if (rc != -EFAULT)
				{
					if (copy_to_user_force((char *) kargs[1], 
							       &retvals->buf, 
							       rc))
						printk("%d: ugh, still can't copy to user \n", current->pid);
				}
			...
			if (kargs[4]) {
				*((int *) kargs[5]) = retvals->addrlen;
				if (copy_to_user ((char *) kargs[4], &retvals->addr, retvals->addrlen)) {
					...
					if (rc != -EFAULT)
					{
						if (copy_to_user_force((char *) kargs[4], 
								       (char *) &retvals->addr,
								       retvals->addrlen))	
							printk("%d: ugh, still can't copy to user \n", current->pid);
					}
		...
		if (retparams) {
			...
			if (retvals->msg_namelen) {
				...
				if (crc) {
					if (rc != -EFAULT)
					{
						if (copy_to_user_force((char *) msg->msg_name,
								       (char *) pdata,
								       retvals->msg_namelen))
						{	
							printk("%d: ugh, still can't copy to user \n", current->pid);
					...
					{
						printk ("Pid %d can't copy msg_namelen %p to %p len %d, rc=%ld\n",
							current->pid, msg->msg_name, pdata, 
							retvals->msg_namelen, crc);
						syscall_mismatch();
					}
				}
			...
			if (retvals->msg_controllen) {
				...
				if (crc) {
					if (rc != -EFAULT)
					{
						if (copy_to_user_force((char *) msg->msg_control,
								       (char *) pdata,
								       retvals->msg_controllen))
						{
					...
					{
						printk ("Pid %d can't copy msg_control %p to %p len %ld, rc=%ld\n", 
			...
			for (i = 0; i < iovlen; i++) {
				...
				if (copy_to_user (msg->msg_iov[i].iov_base, pdata, to_copy)) {
					if (rc != -EFAULT)
					{
						if (copy_to_user_force((char *) msg->msg_iov[i].iov_base,
								       (char *) pdata,
								       to_copy))
						{
	...
	case SYS_GETSOCKOPT:
		if (retparams) {
			...
			if (copy_to_user ((char *) args[4], &retvals->optlen, sizeof(int))) {
				if (rc != -EFAULT)
				{
					if (copy_to_user_force((char *) args[4],
							       (char *) &retvals->optlen,
							       sizeof(int)))
					{
						printk("%d: ugh, still can't copy to user \n", current->pid);
...
replay_ipc (uint call, int first, u_long second, u_long third, void __user *ptr, long fifth)
{
	...
	case SHMAT: 
		if (rc == 0) {
			...
			if (is_preallocated()) {
				...
				if (add_sysv_shm(current->replay_thrd, atretparams->raddr, atretparams->size) < 0)
				{
					printk ("Pid %d: could not add sysv_shm?\n", current->pid);
					return -ENOMEM;
				}
			...
			printk("shmat called with id %d (recid %d), shmaddr 0x%p, shmflg %ld\n", 
	...
	case SHMDT: 
		...
		if (!retval && is_preallocated()) {
			// It looks like rp_sysv_shms isn't used anymore? (I don't see any calls to add)
			u_long size = 0;
			int found = find_and_delete_sysv_shm((u_long) ptr, &size);
			if (found == 0) {
				printk("Pid %d replay shmdt: could not find shm %lx ???\n", current->pid, (u_long) ptr);
				syscall_mismatch();
			}
	...
	case SHMGET: 
		...
		// I think if it fails, I'll just short circuit this stuff
		if (rc < 0) 
		{
			MPRINT("%d shmget failed on rec %ld\n", current->pid, rc);
			return rc;
		}

		temp_shmflags = third & ~IPC_EXCL;

		MPRINT("%d sys_ipc(%u, %i, %lu, %lu, %p, %ld)\n", current->pid, call, first, second, temp_shmflags, ptr, fifth);

		retval = sys_ipc (call, first, second, temp_shmflags, ptr, fifth);		
		if ((rc < 0 && retval >= 0) || (rc >= 0 && retval < 0)) {
			printk ("Pid %d replay_ipc SHMGET, on record we had %ld, but replay we got %ld\n", 
				current->pid, rc, retval);

			
			dump_sysv_mappings(current->replay_thrd);
...
record_clone(unsigned long clone_flags, unsigned long stack_start, struct pt_regs *regs, unsigned long stack_size, int __user *parent_tidptr, int __user *child_tidptr)
{
	...
	struct pthread_timing_log_head __user *timinghead = NULL;
	char __user *timing_start, *old_timing_start = NULL;
	...
	if (!(clone_flags&CLONE_VM)) {
		...
		// I guess I'll do this for the user timings also? 
		timinghead = (struct pthread_timing_log_head __user *) 
			current->record_thrd->rp_pthread_timing_log_addr;

		timing_start = (char __user *) timinghead + sizeof (struct pthread_timing_log_head);
		get_user (old_timing_start, &timinghead->next);
		put_user (timing_start, &timinghead->next);
...
#endif
			// I guess I'll do this for the user timings also?
			tsk->record_thrd->rp_pthread_timing_log_addr =
				current->record_thrd->rp_pthread_timing_log_addr;
			put_user (old_timing_start, &timinghead->next);

		}

		//copy this record hook over...
...
replay_clone(unsigned long clone_flags, unsigned long stack_start, struct pt_regs *regs, unsigned long stack_size, int __user *parent_tidptr, int __user *child_tidptr)
{
	...
	if (rc > 0) {
		...
		// We also need to create a clone here
                MPRINT("replay_clone pre do_fork\n");
		...
		*/
		...
		copy_sysv_shm(current->replay_thrd, tsk->replay_thrd);

		// inherit the parent's app_syscall_addr
		tsk->replay_thrd->app_syscall_addr = current->replay_thrd->app_syscall_addr;

		// inherit the parent's ptrace_syscalls
		tsk->replay_thrd->ptrace_syscall_addr = current->replay_thrd->ptrace_syscall_addr;
                tsk->replay_thrd->ptrace_syscall = current->replay_thrd->ptrace_syscall;
                tsk->replay_thrd->ptrace_pthread_status = current->replay_thrd->ptrace_pthread_status;
                tsk->replay_thrd->ptrace_libc_pthread_status = current->replay_thrd->ptrace_libc_pthread_status;

		// really need to copy the ptrace_outputs
		copy_ptrace_outputs(current->replay_thrd, tsk->replay_thrd);

		// moved to replay_group
		// inherit the paren't status on ignoring segfaults
		//tsk->replay_thrd->ignore_segfaults = current->replay_thrd->ignore_segfaults;

		// inherit the parent's pthread_status
		tsk->replay_thrd->rp_pthread_status = current->replay_thrd->rp_pthread_status; 
		tsk->replay_thrd->rp_libc_pthread_status = current->replay_thrd->rp_libc_pthread_status;


                //do nothing with parents perf_fds and counters (if anything, maybe should close them?)

		// copy this hook over...
		tsk->replay_thrd->rp_replay_hook = current->replay_thrd->rp_replay_hook;

		// not sure this is right
		tsk->replay_thrd->rp_stop_clock_skip = current->replay_thrd->rp_stop_clock_skip;
		...
		if (is_gdb_attached()) {
			...
			// Do the notification now that the child actually is awake
                        int event = (clone_flags & CSIGNAL) == SIGCHLD ? PTRACE_EVENT_FORK : PTRACE_EVENT_CLONE;
			MPRINT("pid %d: Notifying ptrace about clone of %d\n", current->pid,  tsk->pid);

			ptrace_event(event, tsk->pid);
		}
...
shim_clone(unsigned long clone_flags, unsigned long stack_start, struct pt_regs *regs, unsigned long stack_size, int __user *parent_tidptr, int __user *child_tidptr)
{
	...
	if (current->replay_thrd) {
		...
		if (test_app_syscall(120) && test_ptrace_syscall()) {
		...
		// I don't think we should always do this... not sure its my only bug though: 
		if (!(clone_flags & CLONE_VFORK))
		{
			MPRINT("making tsk->replay_thrd NULL, used to be %p",tsk->replay_thrd);
			tsk->replay_thrd = NULL;
			MPRINT("waking up pin thread\n");
			wake_up_new_task(tsk);
		}
...
replay_mremap (unsigned long addr, unsigned long old_len, unsigned long new_len, unsigned long flags, unsigned long new_addr)
{
	...
	if (rc == addr) {
                if (new_len > old_len && is_preallocated()) {
                        int mrc;
                        mrc = sys_munmap(addr + old_len, new_len - old_len);
                        if (mrc < 0) printk("couldn't remove mapping from %lx size %lx (%d\n", addr + old_len, new_len - old_len, mrc);
                }
                printk("replay_mremap o1 %lx %lx %lx %lx %lx\n", addr, old_len, new_len, flags, new_addr);
                retval = sys_mremap (addr, old_len, new_len, flags, new_addr);
        }
	else {
                printk("replay_mremap o2 %lx %lx %lx %lx (%lx) %lx\n",
                       addr, old_len, new_len, flags, flags | MREMAP_FIXED | MREMAP_MAYMOVE, new_addr);
	...
	if (rc != retval) {
                if (IS_ERR((void *) retval)) {
                        MPRINT("%d mremap returns failure: %ld\n", current->pid, PTR_ERR((void *) retval));
                }
...
//		MPRINT("inside of the pin_attached\n");
	...
	else if(is_preallocated ())
	{
		...
		if (rc == -EINTR && current->replay_thrd->rp_pin_attaching) 
		{
			printk("ignore this, someone is attaching\n");
			return rc;
		}
...
replay_vfork_handler (struct task_struct* tsk)
{
	...
	// lets see if we should do this: 
	if (!test_ptrace_syscall())
	{
		MPRINT("skipping replay_vfork_handler pid %d and nullifying %p\n", tsk->pid, tsk->replay_thrd);
		tsk->replay_thrd = NULL;
		return;
	}
...
record_mmap_pgoff (unsigned long addr, unsigned long len, unsigned long prot, unsigned long flags, unsigned long fd, unsigned long pgoff)
{
	...
	struct mmap_pgoff_retvals* recbuf = NULL;
        ...
        // added for omnitable tracking of all mmaped files;
	...
	if ((rc > 0 || rc < -1024)) {
                if ((long)fd > 0) {
                        ...
                        if (vma && rc >= vma->vm_start && vma->vm_file) {
                                ...
                                if (!is_record_cache_file(current->record_thrd->rp_cache_files, fd)) {
                                        char *path = KMALLOC(PATH_MAX +1, GFP_KERNEL);
                                        d_path (&vma->vm_file->f_path, path, PATH_MAX);

                                        recbuf = ARGSKMALLOC(sizeof(struct mmap_pgoff_retvals), GFP_KERNEL);
                                        add_file_to_cache (vma->vm_file, path, &recbuf->dev, &recbuf->ino, &recbuf->mtime);
                                        input.ino = recbuf->ino;
                                        input.dev = recbuf->dev;
                                        input.mtime = recbuf->mtime;
                                        //printk("using recbuf; %lx %lx %lx.%lx\n", input.dev, input.ino,
                                        //input.mtime.tv_sec, input.mtime.tv_nsec);
                                }
                                else {
                                        inode = vma->vm_file->f_dentry->d_inode;
                                        input.ino = inode->i_ino;
                                        input.dev = inode->i_sb->s_dev;
                                        input.mtime = inode->i_mtime;

                                        // I'm pretty weary about the returned mtime in the case
                                        // that it's referencing an older version of a cache_file...?
                                        // FWIW, that branch seems REALLY unlikely during recording.
                                        // I think the program would have to be modifying inodes that it's
                                        // simultaneously trying to cache.
                                        get_cache_file_info(input.dev, input.ino, input.mtime, &input.mtime);
                                        //printk("using gcfn %lx %lx %lx.%lx\n", input.dev, input.ino,
                                        //input.mtime.tv_sec, input.mtime.tv_nsec);
                                }
                                input.prot = prot;
                                input.start = rc; // not technically the input, but same effect
                                input.len = len;
                                input.offset = pgoff;
                                input.clock = current->record_thrd->rp_expected_clock - 1;
                                add_syscall_inputs(current->record_thrd->rp_syscall_inputs, &input);
                        }
                        // the cases when this is an else seem to not actually be file maps?
                        // It's very unclear
...
replay_mmap_pgoff (unsigned long addr, unsigned long len, unsigned long prot, unsigned long flags, unsigned long fd, unsigned long pgoff)
{
	...
	if (IS_ERR((void *) rc)) {
		MPRINT("%d mmap recorded fail: %ld\n", current->pid, PTR_ERR((void *) rc)); 
		return rc;
	}
...
replay_epoll_wait(int epfd, struct epoll_event __user *events, int maxevents, int timeout)
{
	...
	// if (rc == -EINTR && current->replay_thrd->rp_pin_attaching) return rc; not so sure about this...

        if (rc == -EINTR && current->replay_thrd->rp_pin_attaching) {
                // Save parameters so that we can redo  after attach
                return rc;
#if 0
                current->replay_thrd->rp_pin_attach_redo_parms = KMALLOC(sizeof(struct redo_attach_parms), GFP_KERNEL);
                if (current->replay_thrd->rp_pin_attach_redo_parms == NULL) {
                ...
                current->replay_thrd->rp_pin_attach_redo_parms->syscall = 256;
                current->replay_thrd->rp_pin_attach_redo_parms->arg1 = (u_long) epfd;
                current->replay_thrd->rp_pin_attach_redo_parms->arg2 = (u_long) events;
                current->replay_thrd->rp_pin_attach_redo_parms->arg3 = (u_long) maxevents;
                current->replay_thrd->rp_pin_attach_redo_parms->arg4 = (u_long) timeout;
                MPRINT ("epoll attach return %lx\n", current->replay_thrd->rp_saved_rc);
                return current->replay_thrd->rp_saved_rc; // Since pin won't redo system call, use real return code now
#endif
        }
	if (rc > 0) {
		if (copy_to_user (events, retparams, rc * sizeof(struct epoll_event))) {
                        if (copy_to_user_force(events, retparams, rc * sizeof(struct epoll_event)))
...
{
	struct argsalloc_node* node;
	ssize_t copyed = 0, rc = 0, curr_index = 0;
	...
	{
		rc = vfs_write(file, (char *) psr + copyed, sizeof(struct syscall_result)*count - copyed, ppos);
		if (rc < 0) {
		...
		{
			printk ("write_log_data: tried to write %d, got rc 0\n", 
				sizeof(struct syscall_result)*count - copyed);
	...
	list_for_each_entry_reverse (node, &prect->rp_argsalloc_list, list) {
		...
		{
			curr_index = 0;
			while (copyed < data_len)
			{
				rc = vfs_writev (file, pvec + curr_index, kcnt - curr_index, ppos);
				if (rc < 0)
				{
					KFREE (pvec);
					return -EINVAL;
				...
				// short circuit the next bit if we don't need it
				if (copyed >= data_len) break;

				while (curr_index < kcnt && rc < pvec[curr_index].iov_len)
				{
					rc -= pvec[curr_index].iov_len;
					curr_index++;
				}
				pvec[curr_index].iov_base += rc;
				pvec[curr_index].iov_len -= rc;
			}
			kcnt = 0;
		}
	}
	...
	while (copyed < data_len)
	{
                BUG_ON(kcnt - curr_index <= 0);

		rc = vfs_writev (file, pvec + curr_index, kcnt - curr_index, ppos);
		if (rc < 0)
		{
			printk ("write_log_data: writev failed!  got rc %d\n", rc);
			KFREE (pvec);
			return -EINVAL;
		}
		copyed += rc;
		...
		if (copyed >= data_len) break;
                ...
                // chop off all FULL entries that we've written
		while (curr_index < kcnt && rc >= pvec[curr_index].iov_len)
		{
			rc -= pvec[curr_index].iov_len;
			curr_index++;
	...
	MPRINT ("Wrote %d bytes to the file for sysnum %d\n", copyed, psr->sysnum);
	KFREE (pvec);
...
static struct ctl_table replay_ctl[] = {
	...
	{
		.procname	= "record_timings_flag",
		.data		= &record_timings_flag,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= &proc_dointvec,
	},
	{
		.procname	= "skip_write_timings",
		.data		= &skip_write_timings,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= &proc_dointvec,
	},
	{
		.procname	= "timing_buffer_size",
		.data		= &timing_buffer_size,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= &proc_dointvec,
	},
	...
	{
		.procname	= "ptrace_use_dev_null",
		.data		= &ptrace_use_dev_null,
		.maxlen		= sizeof(unsigned int),
		.mode		= 0644,
		.proc_handler	= &proc_dointvec,
	},
