...
//REPLAY -- this isn't ideal, but stupid libc and libpthread linking.. 
#define PTHREAD_LOG_MUTEX_LOCK(lock) if (is_recording()) {		\
...
#define PTHREAD_LOG_MUTEX_UNLOCK(lock) if (is_recording()) {		\
...
__tzname_max ()
{
    ...
    PTHREAD_LOG_MUTEX_LOCK(tzset_lock);

  tzset_internal (0, 0);

  //__libc_lock_unlock (tzset_lock);
  PTHREAD_LOG_MUTEX_UNLOCK(tzset_lock);
