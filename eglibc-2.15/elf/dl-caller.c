...
// Very hacky to include the version number in this. but it doesn't seem to work!?

# define LIBDL_CORRECT_VERSION_SO "libdl-2.15.so"
# define LIBDL_CORRECT_SO "libdl.so"

int
attribute_hidden
check_suffix(const char* full, const char*suffix)
{
...
_dl_check_caller (const void *caller, enum allowmask mask)
{
  ...
  // omniplay bullshit
  static const char expectedOT1[] = LIBDL_CORRECT_SO;
  static const char expectedOT2[] = LIBDL_CORRECT_VERSION_SO;
#ifdef LIBPTHREAD_SO
  ...
  for (Lmid_t ns = 0; ns < GL(dl_nns); ++ns)
	  ...
	  && caller < (const void *) l->l_text_end)
	{
	  ...
	  if ((mask & allow_libdl) && (strcmp (expected2, l->l_name) == 0 ||
                                       check_suffix(l->l_name, expectedOT1) == 0 ||
                                       check_suffix(l->l_name, expectedOT2) == 0 ||
                                       check_suffix(l->l_name, expected2)))
              return 0;
#ifdef LIBPTHREAD_SO
