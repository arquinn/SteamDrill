...
#ifdef USE_TIMING
#define PTHREAD_TIMING_LOG_ENTRIES (1024 * 1024)
#define TIMING_LOG_STEP 1000
...
struct pthread_timing_log_entry {
    unsigned long clock; //same val as in recording? 
    unsigned long long timeval; //time in ns
};
...
static inline int is_recording (void)
{
    ...
    if (pthread_log_status == PTHREAD_LOG_SKIP)   return 0;
...
static inline int is_skip(void)
{
    if (pthread_log_status == PTHREAD_LOG_SKIP) return 1;
    return 0;
}
...
static inline void pthread_log_debug(const char* fmt,...)
{
    va_list ap;
    char buffer[256] = {0,};
