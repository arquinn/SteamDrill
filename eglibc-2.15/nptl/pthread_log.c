...
#include <fcntl.h>
#include <unistd.h>
#include <signal.h> // can do?
#include <linux/perf_event.h> // can do?
...
#define __NR_pthread_mmap_log  130
#define __NR_pthread_watchdog  222

#define __NR_perf_event_open 336

#ifdef USE_TIMING
#define __NR_pthread_timing_log 169
#define __NR_pthread_timing_log_full 188
#endif
...
{
    INTERNAL_SYSCALL_DECL(__err);
    return INTERNAL_SYSCALL(perf_event_open, __err, 5, event, 0, -1, -1, 0);
}


void pthread_watchdog (void)
{
    pid_t pid;
    INTERNAL_SYSCALL_DECL(__err);
    pid = INTERNAL_SYSCALL(pthread_watchdog, __err, 0);

    // if we get the greenlight for a watchdog, set it up (!)
    if (pid > 0)
    {
        struct pthread_log_head* head = THREAD_GETMEM (THREAD_SELF, log_head);
        struct perf_event_attr pe;
        int rc = 0, watchdog_fd = 0;
        ...
        pe.exclude_kernel = 1;
        pe.sample_period = 0x100000; // many billion instructions.
        pe.disabled = 1;

        head->ignore_flag = 1; // this might need to go up top... not sure how that gettid works
        pthread_log_debug("starting watchdog timer");
        watchdog_fd = perf_event_open(&pe);
        if (watchdog_fd < 0) {
            printf("cannot create watchdog");
        }

        rc = fcntl(watchdog_fd, F_SETOWN, pid);
        if (rc) {
            pthread_log_debug("couldn't setown?  %d", rc);
        }
        rc = fcntl(watchdog_fd, F_SETFL, O_ASYNC);
        if (rc) {
            printf("can't setup O_ASYNC? %d", rc);
        }
        rc = fcntl(watchdog_fd, F_SETSIG, SIGKILL);
        if (rc) {
            pthread_log_debug("can't setup sigkill? %d", rc);
        }
        rc = ioctl(watchdog_fd, PERF_EVENT_IOC_ENABLE, 0);
        if (rc) {
            DPRINT("cannot eanble watchdog! %d", rc);
        }
        head->ignore_flag = 0;
    }
}
...
static void pthread_log_init (void)
{
    ...
    rc = close(fd); //can I do this?
    if (rc < 0)
    {
	pthread_log_debug("couldn't close shm_path?\n");
	exit (0);
    }
...
{
    DPRINT("pthread_log_do_tick %d",pthread_log_status);
    if (is_recording()) { 
	struct pthread_log_head* head = THREAD_GETMEM (THREAD_SELF, log_head);
	GET_OLD_STACKP();
	SET_NEW_STACKP();
...
void pthread_log_mutex_lock_rec (volatile __libc_lock_t* lock)
{
...
void pthread_log_mutex_lock_rep (volatile __libc_lock_t* lock)
{
...
void pthread_log_mutex_lock (__libc_lock_t* lock)
{
    if (is_recording() || is_replaying()) {
        struct pthread_log_head* head = THREAD_GETMEM (THREAD_SELF, log_head);
        THREAD_SETMEM(THREAD_SELF, stackswitch1, (u_long) lock);

        GET_OLD_STACKP();
        SET_NEW_STACKP();
        DPRINT("pthread_log_mutex_lock %p osp 0x%lx old_stack 0x%lx\n",
               head, head->old_stackp, *(u_long*)head->old_stackp);

        lock = (__libc_lock_t *) THREAD_GETMEM(THREAD_SELF, stackswitch1);
        if (is_recording())
            pthread_log_mutex_lock_rec (lock);
        else
            pthread_log_mutex_lock_rep (lock);
        RESET_OLD_STACKP();
        DPRINT("pthread_log_mutex_lock %p *old_stack 0x%lx\n", head, *(u_long*)head->old_stackp);
...
int check_recording (void)
{
...
#ifdef USE_TIMING
    {
	struct pthread_timing_log_head* timehead = THREAD_GETMEM (THREAD_SELF, timing_log_head);
	pthread_timing_log_full();
	timehead->next = ((char *) head + sizeof (struct pthread_timing_log_head));
	DPRINT ("check_recording reload log %p\n", timehead);
    }
#endif
	...
	// at the cost of an extra system call, we can make this work for our ptrace tool
    ...
    if (pthread_log_status == PTHREAD_LOG_SKIP)
	return PTHREAD_LOG_SKIP;
...
#ifdef USE_TIMING
    {
	struct pthread_timing_log_head* timehead;
	timehead = allocate_timing_log();
	THREAD_SETMEM (THREAD_SELF, timing_log_head, timehead);
	register_timing_log(timehead);
	DPRINT ("Allocated log for timing thread %p\n", timehead);
...
allocate_log (void)
{
    ...
    INTERNAL_SYSCALL_DECL(__err);
    head = (struct pthread_log_head *) INTERNAL_SYSCALL(pthread_mmap_log, __err, 4, 0, size, PROT_READ|PROT_WRITE, MAP_PRIVATE|MAP_ANONYMOUS);
...
#ifdef USE_TIMING
    struct pthread_timing_log_head *timing_head = THREAD_GETMEM (THREAD_SELF, timing_log_head);
    register_timing_log(timing_head);
    DPRINT("set timing log in register_log %p\n", timing_head);
#endif

    DPRINT ("Registered ignore flag at %p value %d head %p\n", &head->ignore_flag, head->ignore_flag, head);
}

#ifdef USE_TIMING
struct pthread_timing_log_head* allocate_timing_log(void) 
{	
       struct pthread_timing_log_head* head;    
       u_long size = sizeof(struct pthread_timing_log_head) + PTHREAD_TIMING_LOG_ENTRIES * sizeof(struct pthread_timing_log_entry);
       if (size % 4096) size += 4096 - (size%4096);

       INTERNAL_SYSCALL_DECL(__err);
       head = (struct pthread_timing_log_head *) INTERNAL_SYSCALL(pthread_mmap_log, __err, 4, 0, size, 
								  PROT_READ|PROT_WRITE, 
								  MAP_PRIVATE|MAP_ANONYMOUS);
       if (head == MAP_FAILED) {
	   pthread_log_debug ("Unable to allocate record thread log, errno is %d\n", errno);
	   abort ();
       }
       
       head->next = ((char *) head + sizeof (struct pthread_timing_log_head));       
       head->end = (char *) head + size;
       head->elapsed = 0;

       DPRINT("allocate_timing_log %p next %p end %p\n", head, head->next, head->end);
       return head;
}

void register_timing_log(struct pthread_timing_log_head *log_addr)
{
    INTERNAL_SYSCALL_DECL(__err);
    INTERNAL_SYSCALL(pthread_timing_log,__err, 1, log_addr);
}


// This informs the kernel that the log is full
void pthread_timing_log_full (void)
{
    INTERNAL_SYSCALL_DECL(__err);
    INTERNAL_SYSCALL(pthread_timing_log_full,__err,0);
}
...
void record_timing(unsigned long clock)
{
    uint64_t time = rdtscp();
#ifndef SKIP_LOG_TIMING
    struct pthread_timing_log_head *head = THREAD_GETMEM (THREAD_SELF, timing_log_head);
    struct pthread_timing_log_entry *entry = head->next;

    DPRINT("record_timing: %p %p %p\n", head, head->next, head->end);

    if (time > head->elapsed + TIMING_LOG_STEP)
    {
	// precheck to be safe:
	if (head->next + sizeof(struct pthread_timing_log_entry) >= head->end) {
	    DPRINT("record_timing need to flush: %p %p %p\n", head, head->next, head->end);
	    pthread_timing_log_full();
	    head->next = ((char *) head + sizeof (struct pthread_timing_log_head));
	    entry = head->next;
	    DPRINT("record_timing: %p %p %p\n", head, entry, head->end);
	...
	entry->clock = clock;
	entry->timeval = time;
	head->next += sizeof(struct pthread_timing_log_entry);
	head->elapsed = time;
    }
#endif
}

//TODO: How do I manage these on replay? 
//void replay_timing()
...
pthread_log_replay (unsigned long type, unsigned long check)
{
    ...
    if (is_skip())
    {
	DPRINT("huh, pthread_log_replay should have skipped?? call from %lx\n", __builtin_return_address(1)); //not sure this works?
	return 0;
    }
	...
	pthread_log_debug ("Replay mismatch: log record %lu (curr %lu) has type %lu check %x - but called with type %lu check %lx\n", data->clock, *ppthread_log_clock, data->type, data->check, type, check);
#ifdef USE_EXTRA_DEBUG_LOG
...
pthread_log_record (int retval, unsigned long type, unsigned long check, int is_entry)
{
    ...
    DPRINT("pthread_log_record started with %lu\n", type);
...
pthread_log_replay (unsigned long type, unsigned long check)
{
	...
	head->num_expected_records--;
        ...
        // check for watchdog here:
        if (head->num_expected_records == 1 && (*((unsigned long*)head->next) & FAKE_CALLS_FLAG)) {
            unsigned long *cpentry = (unsigned long*) head->next;
            unsigned long *next = (unsigned long*)(head->next + sizeof(unsigned long));

            if (*cpentry & SKIPPED_CLOCK_FLAG) {
                next += 1;
            }
            if (*cpentry & NONZERO_RETVAL_FLAG) {
                next += 1;
            }
            if (*cpentry & ERRNO_CHANGE_FLAG) {
                next += 1;
            }
            if (*cpentry & FAKE_CALLS_FLAG && *next == WATCHDOG_COUNT) {
                pthread_log_debug("this looks like a time for the watchdog.\n");
                pthread_watchdog();
            }
        }
	return 0;
	...
	DPRINT ("waiting for clock %lu (type %lx, check %lx, status %x, is_replaying() %x)\n", next_clock, type, check, pthread_log_status, is_replaying());
...
#endif
    (*ppthread_log_clock)++;
    DPRINT ("Replay clock incremented to %d, num headphakes %d\n", *ppthread_log_clock, num_fake_calls);
    if (head->next >= head->end) {
	DPRINT ("Log is full - need to reload\n");
	pthread_log_full ();
	head->next = ((char *) head + sizeof (struct pthread_log_head));
     ...
     pthread_mutex_t *mutex;
{
    ...
    DPRINT("pthread_mutex_consistent %d",pthread_log_status);
    if (is_recording() || is_replaying()) {
        struct pthread_log_head* head = THREAD_GETMEM (THREAD_SELF, log_head);
     ...
     int *prioceiling;
{
  ...
  DPRINT("pthread_mutex_getprioceiling %d",pthread_log_status);

  if (is_recording() || is_replaying()) {
      struct pthread_log_head* head = THREAD_GETMEM (THREAD_SELF, log_head);
      THREAD_SETMEM(THREAD_SELF, stackswitch1, (u_long) mutex);
      THREAD_SETMEM(THREAD_SELF, stackswitch2, (u_long) prioceiling);

      GET_OLD_STACKP();
      SET_NEW_STACKP();
      mutex = (pthread_mutex_t *) THREAD_GETMEM(THREAD_SELF, stackswitch1);
      prioceiling = (int *) THREAD_GETMEM(THREAD_SELF, stackswitch2);
      if (is_recording())
     ...
     int *old_ceiling;
{
  ...
  DPRINT("pthread_mutex_setprioceiling %d",pthread_log_status);
  if (is_recording() || is_replaying()) {
      struct pthread_log_head* head = THREAD_GETMEM (THREAD_SELF, log_head);
      THREAD_SETMEM(THREAD_SELF, stackswitch1, (u_long) mutex);
      THREAD_SETMEM(THREAD_SELF, stackswitch2, (u_long) prioceiling);
      THREAD_SETMEM(THREAD_SELF, stackswitch3, (u_long) old_ceiling);

      GET_OLD_STACKP();
      SET_NEW_STACKP();
      mutex = (pthread_mutex_t *) THREAD_GETMEM(THREAD_SELF, stackswitch1);
      prioceiling = (int) THREAD_GETMEM(THREAD_SELF, stackswitch2);
      old_ceiling = (int*) THREAD_GETMEM(THREAD_SELF, stackswitch3);
      if (is_recording())
...
{
    int rc;
    DPRINT("pthread_mutex_timedlock %d",pthread_log_status);
     ...
     pthread_barrier_t *barrier;
{
  ...
  DPRINT("pthread_barrier_destroy %d",pthread_log_status);
     ...
     pthread_barrier_t *barrier;
{
  ...
  DPRINT("pthread_barrier_wait %d",pthread_log_status);
...
{
  int rc;
  DPRINT("pthread_cond_destroy %d", pthread_log_status);
...
{
    int rc;
    DPRINT("pthread_spin_lock %d",pthread_log_status);
    ...
    if (is_recording() || is_replaying()) {
        ...
        lock = (pthread_spinlock_t *) THREAD_GETMEM(THREAD_SELF, stackswitch1);
        if (is_recording())
...
int __new_sem_post (sem_t *__sem)
{
    ...
    if (is_recording() || is_replaying()) {
        struct pthread_log_head* head = THREAD_GETMEM (THREAD_SELF, log_head);
        THREAD_SETMEM(THREAD_SELF, stackswitch1, (u_long) __sem);

        GET_OLD_STACKP();
        SET_NEW_STACKP();
        __sem = (sem_t *) THREAD_GETMEM(THREAD_SELF, stackswitch1);
        if (is_recording())
...
int sem_timedwait (sem_t *__sem, const struct timespec *abstime)
{
    ...
    if (is_recording() || is_replaying()) {
        ...
        SET_NEW_STACKP();
        __sem = (sem_t *) THREAD_GETMEM(THREAD_SELF, stackswitch1);
        abstime = (struct timespec *) THREAD_GETMEM(THREAD_SELF, stackswitch2);
        if (is_recording())
...
{
    int i, rc, found = 0;
    struct my_once_lock* tmp, *prev_tmp, *search_tmp;

//    pthread_log_debug("starting pthread_once_rec\n");

    head->ignore_flag = 1; // don't record this private lock
    
    // Get a per-control lock
  try_again:
//    pthread_log_debug("0: locking %p", &once_lock);
    lll_lock (once_lock, LLL_PRIVATE);
//    pthread_log_debug("0: locked once_lock %p", &once_lock);
    
    if (!my_once_init) {
//	pthread_log_debug("start my_once_init\n");
	    // Set up free list
	    ...
	    my_once_init = 1;
//	    pthread_log_debug("end my_once_init\n");
    }
...
/*    count = 0;
    ...
    if (once_head && once_head != once_head->next)
    {
	for (tmp = once_head; 
	     tmp || count > MY_ONCE_ARRAY_SIZE; 
	     tmp = tmp->next) {
	    count++;
	}
	...
	if (count >= 50) 
	{
		...
		count++;
		pthread_log_debug("used_list %d: %p\n",count, tmp);
	    }
	}
    }
*/
...
//    pthread_log_debug("start for loop once_head %p\n", once_head);
//    if (once_head)
//	pthread_log_debug("next %p\n", once_head->next);

    for (tmp = once_head; tmp; tmp = tmp->next) {
      if (tmp->control == once_control) {
	tmp->refcnt++;
//	pthread_log_debug("1: dropping %p, locking %p\n", &once_lock, &(tmp->lock) );
//	pthread_log_debug("found %p, next %p\n",tmp,tmp->next);
...
//    pthread_log_debug("after for loop\n");
    if (!found) {
//	pthread_log_debug("start !found\n");
      prev_tmp = NULL;
      tmp = once_free;
      if (tmp == NULL) {
      ...
      once_free = tmp->next;
//      pthread_log_debug("once_free now %p (was %p)\n",once_free, tmp);
      ...
      tmp->next = once_head;
      once_head = tmp;
//      pthread_log_debug("once_head set 1 %p (was %p)\n", once_head, tmp->next);
      //    pthread_log_debug("2: dropping %p, locking %p\n", &once_lock, &(tmp->lock) );

      lll_lock (tmp->lock, LLL_PRIVATE);
      lll_unlock (once_lock, LLL_PRIVATE);
//      pthread_log_debug("end !found %p %p\n", tmp, tmp->next);
    }
...
//    pthread_log_debug("tmp %p next %p\n",tmp, tmp->next);
    // Release the per-control lock
//    pthread_log_debug("3: dropping %p, locking %p\n", &(tmp->lock), &(once_lock));
	...
	//figure out where in the once_head tmp is: 
	prev_tmp = NULL;
	for (search_tmp = once_head; search_tmp; search_tmp = search_tmp->next) {
	    if (search_tmp == tmp)
	    {
		found = 1;
		break;
	...
	if (!found) 
	    pthread_log_debug ("Couldn't find tmp in the used list...?\n");


      if (prev_tmp) {
	prev_tmp->next = tmp->next;
//	pthread_log_debug("set %p nxt %p\n",prev_tmp, prev_tmp->next);
      } else {
	  // there's an assumption here that tmp is the head. 
	once_head = tmp->next;
//	pthread_log_debug("once_head set 2 %p\n", once_head);
      }
      // Put back on free list
      tmp->next = once_free; //this is the broken set
      once_free = tmp;
//      pthread_log_debug("2: set %p nxt %p\n",tmp, tmp->next);
    }
//    pthread_log_debug("4: dropping %p", &once_lock);
    lll_unlock (once_lock, LLL_PRIVATE);

    head->ignore_flag = 0;

//    pthread_log_debug("ending pthread_once_rec\n");
    return rc;
}
     ...
     void (*init_routine) (void);
{
  ...
  DPRINT("pthread_once %d",pthread_log_status);
...
{
    DPRINT("pthread_log_lll_lock %d",pthread_log_status);
...
{
  DPRINT("pthread_log_lll_unlock %d",pthread_log_status);
  if (is_recording()) {
...
{
  DPRINT("pthread_log_lll_wait_tid %d",pthread_log_status);
  if (is_recording()) {
...
{
  int rc;
  DPRINT("pthread_log_lll_tiemdwait_tid %d",pthread_log_status);
...
int pthread_log__sync_read(volatile int* val)
{
  ...
  DPRINT("pthread_log__sync_read %d",pthread_log_status);
...
int pthread_log__sync_add_and_fetch(int* val, int x)
{
  ...
  DPRINT("pthread_log__sync_and_fetch %d",pthread_log_status);
...
int pthread_log__sync_sub_and_fetch(int* val, int x)
{
  struct pthread_log_head* head;
  int ret;
  DPRINT("pthread_log__sync_sub_and_fetch %d",pthread_log_status);
  if (is_recording()) { 
    head = THREAD_GETMEM (THREAD_SELF, log_head);
...
int pthread_log__sync_fetch_and_add(int* val, int x)
{
  struct pthread_log_head* head;
  int ret;
  DPRINT("pthread_log__sync_fetch_and_add %d",pthread_log_status);
  if (is_recording()) { 
    head = THREAD_GETMEM (THREAD_SELF, log_head);
...
int pthread_log__sync_fetch_and_sub(int* val, int x)
{
  struct pthread_log_head* head;
  int ret;
  DPRINT("pthread_log__sync_fetch_and_sub %d",pthread_log_status);
...
int pthread_log__sync_lock_test_and_set(int* val, int x)
{
  struct pthread_log_head* head;
  int ret;
  DPRINT("pthread_log__sync_test_and_set %d",pthread_log_status);
  if (is_recording()) { 
    head = THREAD_GETMEM (THREAD_SELF, log_head);
