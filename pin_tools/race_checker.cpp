...
bool detect_race(THREADID tid, VOID *ref_addr, ADDRINT size, ADDRINT ip, int ref_type) {
  ...
  /*
  PIN_LockClient();
  bool funccity = IMG_Valid(IMG_FindByAddress(ip)) ?
      IMG_Name(IMG_FindByAddress(ip)).find("libpthread.so.0") != string::npos ||
      IMG_Name(IMG_FindByAddress(ip)).find("libc.so.6") != string::npos ||
      IMG_Name(IMG_FindByAddress(ip)).find("ld-linux.so.2") != string::npos : false;
  PIN_UnlockClient();

  if(num_threads == 1) { // || funccity) {
        return false;
  }
  */
    if(!inrange())
	...
	// Update varible access history and check for violations
	    ...
	    PIN_LockClient();
            ...
            //if (img.find("libpthread.so.0") != string::npos ||
            //img.find("libc.so.6") != string::npos){
            //PIN_UnlockClient(); // ahh!
            //return false;
            //}

            fprintf(stderr, "@@@ race ip %#x, addr %p, clock %lu\n", ip, ref_addr, get_clock_value(fd));
            fprintf(stderr, "%s static %#x\n", img.c_str(), find_static_address(ip));
					    ...
					    ADDRINT syscallarg3, ADDRINT syscallarg4, ADDRINT syscallarg5)
{
    ...
    fprintf (stderr, "Pid %d, tid %d, (record pid %d), syscall num is %d clock %lu\n",
             PIN_GetPid(), PIN_GetTid(), get_record_pid(), (int) syscall_num, get_clock_value(fd));
...
void record_locked (ADDRINT ip)
{
	...
	PIN_LockClient();
        ...
        if (img.find("libpthread.so.0") == string::npos  && img.find("libc.so.6") == string::npos) {
