...
{	    
    ...
    if (i == 0xc030bb6d) cerr << element->second << endl;
...
static void print_input(nodeid i)
{
	...
	print_node(i, entry->imm_info.instcount, entry->imm_info.ip, 0,0,0,0); //chances of i being a problem are zip
	break;
    case input_info_type::READ:
	cerr << hex << "found a read one... " << i << " cnt " << dec << entry->syscall_cnt << endl;
	print_read_node(i, i, entry->syscall_info.fileno, entry->syscall_cnt, entry->type);
	break;
    case input_info_type::MMAP:
	cerr << hex <<"found a mmap one... " << i << " cnt " << dec << entry->syscall_cnt << endl;
	print_read_node(i, i, entry->syscall_info.fileno, entry->syscall_cnt, entry->type);
	break;
...
//	u_long bufaddr = *outputPtr;
	...
	if (desired_output && buf[0] == desired_output) 
	{
	    ...
	    print_node(newnodeid, (-10 * newnodeid), ip, buf[0], 0, 0, 0);
	    newnodeid--;
