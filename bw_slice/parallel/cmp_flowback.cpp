...
{
    CF::condensed_flowback cf; 
    for (auto &i : *nodeids) 
    {      
	if (i < LOCAL_SOURCE_OFFSET) { 
	    cerr << "local source found" << i << std::endl;
	    cf.add_input_node(NULL, i, true);
	}
	else if (i < NODE_OFFSET) {
	    cerr << "global source found " << i << std::endl;

	    auto element = inputMap.find(i);
	    if (element == inputMap.end()) 
		cerr << "huh, can't find input " << i << endl;

	    cf.add_input_node(element->second, i);
	}
	else { 	
    ...
    file << cf; 
    file.close();
    cerr << "finished dumping nodeids" << endl;
...
#ifdef TRACE
    *trace_f << "added " << inputMap.size() << " inputs" << std::endl;
#endif
}
...
mergelog_iter_depth(nodeid id,  set<nodeid> *nodeids, stacktype stack,
		    u_int32_t &bucket_cnt, uint32_t &bucket_stop, bool verbose)
{
    nodeids->insert(id);
    if (verbose) cout << hex << id << endl; 
	...
	do { 
		...
		if (verbose) cout << hex << id << "<-"<<entry->p1<<endl;
		...
		if (verbose) cout << hex << id << "<-"<<entry->p2<<endl;
...
//		     << curr_ii->imm_info.instcount << endl; 
		if (outputPtr >= outStop) 
		    mergelog_iter_depth(buf[i], nodeids, *stack, wbucket_cnt, wbucket_stop, true);  
		else 
		    mergelog_iter_depth(buf[i], nodeids, *stack, wbucket_cnt, wbucket_stop, false);  
	    }
	}
    }   
...
do_addresses (void* pdata)
{
	...
	if (iter == paddress_map->end()) {
	    ...
	    mergelog_iter_depth(iter->second, nodeids, *stack, wbucket_cnt, wbucket_stop, false);
	}
    }    
