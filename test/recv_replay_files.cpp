...
int sync_logfiles (int s)
{
	...
	for (u_long i = 0; i < ccnt; i++) {
		...
		creply[i] = true;
            ...
            creply[i] = true; //safest bet is just to always say we want cachefiles
	}
	...
	if (creply[i]) {
	    ...
	    char cname[PATHLEN], crname[PATHLEN], crnamemap[PATHLEN+5],
                newname[PATHLEN], mapname[PATHLEN+5];
	    struct stat64 st;
	    sprintf (cname, "/replay_cache/%x_%x", ci[i].dev, ci[i].ino);
            sprintf (mapname, "%s.orig", cname);
		...
		if (st.st_mtim.tv_sec > ci[i].mtime.tv_sec ||
			...
			return rc;
                    ...
                    sprintf (mapname, "%s.orig", newname);
		} else {
			...
			return rc;
                    ...
                    sprintf (crnamemap, "%s.orig", crname);
		    rc = rename (mapname, crnamemap);
		    if (rc < 0) {
			fprintf (stderr, "Cannot rename cache file %s to %s, rc=%d\n", cname, crname, rc);
			return rc;
		    }
	    ...
	    rc = fetch_file (s, "/replay_cache");
	    if (rc < 0) return rc;
            rc = rename ("/replay_cache/rename_me", mapname);
            if (rc < 0) {
              fprintf (stderr, "Cannot rename temp cache file to %s, rc=%d\n", mapname, rc);
              return rc;
            }
