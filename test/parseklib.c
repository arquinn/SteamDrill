...
static u_long consume_getretparams_epoll_wait(struct klogfile *klog,
                                              struct klog_result *res);
	...
	if (res->psr.flags & SR_HAS_RETPARAMS) {
		...
		if (log->parse_rules[psr->sysnum]->get_retparamsize) {
			...
			ret = log->parse_rules[psr->sysnum]->retparamsize;
                        ...
                        if (log->simple) {
#ifdef USE_BR
                                br_consume(log->br, ret);
#else
                                if (lseek(log->fd, ret, SEEK_CUR) < 0) {
                                        perror("lseek: ");
                                        return -1;
                                }
#endif
...
static int read_psr_chunk(struct klogfile *log) {
...
#ifdef USE_BR
                rc = br_read(log->br, &count, sizeof(count));
#else
...
#ifdef USE_BR
                rc = br_read(log->br, psrs, sizeof(struct syscall_result) * count);
#else
...
#ifdef USE_BR
                rc = br_read(log->br, &data_size, sizeof(data_size));
#else
	...
	for (i = 0; i < count; i++) {
		...
		if ((apsr->psr.flags & SR_HAS_START_CLOCK_SKIP) != 0) {
			u_long clock;
#ifdef USE_BR
                        rc = br_read (log->br, &clock, sizeof(u_long));
#else
...
#ifdef USE_BR
                        rc = br_read(log->br, &apsr->retval, sizeof(long));
#else
		...
		if (log->printfcns[apsr->psr.sysnum]) {
			...
			apsr->printfcn = log->default_printfcn;
                ...
                // this essentially reads everything twice (!)
		apsr->retparams_size = getretparamsize(log, apsr);
                assert(apsr->retparams_size >= 0);

                if (log->simple) {
                        apsr->retparams_size = 0;
                        // simple mode doesn't deal w/ these
                }
                else if (apsr->retparams_size > 0) {
			long rc;
			...
			do {
#ifdef USE_BR
                                rc = br_read(log->br, apsr->retparams+bytes_read, apsr->retparams_size-bytes_read);
#else
...
#ifdef USE_BR
                                rc = br_read(log->br, &apsr->signal->raw, 172);
#else
...
#ifdef USE_BR
                        debugf("log (%d) from %ld\n", i, br_offset(log->br));
#else
                        debugf("log (%d) from %ld\n", i, lseek(log->fd, 0, SEEK_CUR));
#endif
...
struct klogfile *parseklog_open(const char *filename) {
	...
	/* Open the file and initialize the fd */
        ret->fd = open(filename, O_RDONLY | O_LARGEFILE);
        if (ret->fd < 0) {
		perror("open: ");
		goto out_free;
        }
...
static void add_simple_parse_rule_exceptions(struct klogfile *log);
struct klogfile *parseklog_open_simple(const char *filename) {
  struct klogfile *ret = parseklog_open(filename);
  assert (ret);

  /* Set up the parse rules */
  memset(ret->parse_rules, 0, sizeof(ret->parse_rules));
  add_simple_parse_rule_exceptions(ret);

  ret->simple = 1;

  // should have some solution for using the br (if needed)
  return ret;
}
...
static u_long consume_varsize(struct klogfile *klog, struct klog_result *res) {
	u_long val;

#ifdef USE_BR
        if (br_read (klog->br, &val, sizeof(u_long)) != sizeof(u_long)) {
#else
        if (read (klog->fd, &val, sizeof(u_long)) != sizeof(u_long)) {
#endif
...
#ifdef USE_BR
        br_consume(klog->br, val & sizeof(u_short));
#else
        if (lseek(klog->fd, val * sizeof(u_short), SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
#endif
...
static u_long consume_getretparams_retval(struct klogfile *klog,
		struct klog_result *res) {
#ifdef USE_BR
        br_consume(klog->br, res->retval);
#else
        if (lseek(klog->fd, res->retval, SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
#endif
...
static u_long consume_getretparams_read(struct klogfile *log, struct klog_result *res) {
	long rc;
	u_long size = 0;
	u_int is_cache_read;

#ifdef USE_BR
        rc = br_read(log->br, &is_cache_read, sizeof(u_int));
#else
	rc = read(log->fd, &is_cache_read, sizeof(u_int));
#endif
	if (rc != sizeof(u_int)) {
...
#ifdef USE_BR
        br_consume(log->br, size);
#else
        if (lseek(log->fd, size, SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
#endif
...
static u_long consume_getretparams_write(struct klogfile *klog,
                                         struct klog_result *res) {
	long rc;
	u_long size = 0;
	int is_shared;
	...
	// size += sizeof(int);
#ifdef USE_BR
        rc = br_read(klog->br, &is_shared, sizeof(int));
#else
        rc = read(klog->fd, &is_shared, sizeof(int));
#endif
	if (rc != sizeof(int)) {
...
#ifdef USE_BR
        br_consume(klog->br, size);
#else
        if (lseek(klog->fd, size, SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
        #endif
...
static u_long consume_getretparams_getgroups(struct klogfile *klog,
		struct klog_result *res) {

#ifdef USE_BR
        br_consume(klog->br, sizeof(u_short) * res->retval);
#else
        if (lseek(klog->fd, sizeof(u_short) * res->retval, SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
#endif
...
static u_long consume_getretparams_getgroups32(struct klogfile *klog,
		struct klog_result *res) {

#ifdef USE_BR
        br_consume(klog->br, sizeof(gid_t) * res->retval);
#else
        if (lseek(klog->fd, sizeof(gid_t) * res->retval, SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
#endif
...
static u_long consume_getretparams_io_getevents(struct klogfile *klog,
		struct klog_result *res) {

#ifdef USE_BR
        br_consume(klog->br, 32 * res->retval);
#else
        if (lseek(klog->fd, 32 * res->retval, SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
#endif
...
#ifdef USE_BR
        br_consume(klog->br, sizeof(struct epoll_event) * res->retval);
#else
        if (lseek(klog->fd, sizeof(struct epoll_event) * res->retval, SEEK_CUR) < 0) {
                perror("lseek: ");
                return -1;
        }
#endif
	...
	switch (call) {
		...
		case SYS_RECVMSG: {
...
#ifdef USE_BR
                                rc = br_read(log->br, ((char *)&msg) + sizeof(int), sizeof(struct recvmsg_retvals) - sizeof(int));
#else
			...
			if (rc != sizeof(struct recvmsg_retvals) - sizeof(int)) {
                                ...
                                fprintf(stderr, "hmph. returned %ld: try something:\n", rc);
                                rc = read(log->fd, ((char *)&msg) + sizeof(int), sizeof(struct recvmsg_retvals) - sizeof(int));
				fprintf(stderr, "cannot read recvfrom values rc %lu\n", rc);
				return -1;
			}
		...
		case SYS_RECVMMSG: {
			if (res->retval > 0) {
				long len;
#ifdef USE_BR
                                rc = br_read(log->br, ((char *)&len), sizeof(long));
#else
...
static u_long consume_getretparams_socketcall(struct klogfile *log, struct klog_result *res) {
	int call;
	long rc;
	u_long size = 0;
...
#ifdef USE_BR
        rc = br_read(log->br, &call, sizeof(int));
#else
	rc = read(log->fd, &call, sizeof(int));
#endif
	...
	switch (call) {
		...
		case SYS_GETPEERNAME: {
			struct accept_retvals avr;
#ifdef USE_BR
                        rc = br_read(log->br, ((char *) &avr) + sizeof(int),
                                     sizeof(struct accept_retvals) - sizeof(int));
#else
                        rc = read(log->fd, ((char *) &avr) + sizeof(int),
                                  sizeof(struct accept_retvals) - sizeof(int));
		...
		case SYS_GETSOCKOPT: {
			struct getsockopt_retvals sor;
#ifdef USE_BR
                        rc = br_read (log->br, ((char *) &sor) + sizeof(int),
                                      sizeof(struct getsockopt_retvals) - sizeof(int));
#else
                        rc = read (log->fd, ((char *) &sor) + sizeof(int),
                                   sizeof(struct getsockopt_retvals) - sizeof(int));
...
static u_long consume_getretparams_pread64 (struct klogfile *log, struct klog_result *res) 
{
	long rc;
	u_long size = 0;
	int extra_bytes = 0;
...
#define _DEFRULE_CONSUME(sysnr, default, fcn ) \
	static struct parse_rules exception_consume_##sysnr = {        \
		.get_retparamsize = (fcn),                            \
		.retparamsize = (default)                             \
	};                                                     \


#define DEFRULE(sysnr, size) _DEFRULE(sysnr, size, NULL) _DEFRULE_CONSUME(sysnr, size, NULL)
#define DEFRULE_FCN(sysnr, fcn) _DEFRULE(sysnr, 0, fcn) _DEFRULE_CONSUME(sysnr, 0, consume_##fcn)

#define ADDRULE(sysnr, log) log->parse_rules[sysnr]=&exception_##sysnr
#define ADDRULE_SIMPLE(sysnr, log) log->parse_rules[sysnr]=&exception_consume_##sysnr
...
static void add_simple_parse_rule_exceptions(struct klogfile *log) {
	ADDRULE_SIMPLE(3, log);
	ADDRULE_SIMPLE(4, log);
	ADDRULE_SIMPLE(5, log);
	ADDRULE_SIMPLE(7, log);
	ADDRULE_SIMPLE(11, log);
	ADDRULE_SIMPLE(13, log);
	ADDRULE_SIMPLE(18, log);
	ADDRULE_SIMPLE(28, log);
	ADDRULE_SIMPLE(42, log);
	ADDRULE_SIMPLE(43, log);
	ADDRULE_SIMPLE(54, log);
	ADDRULE_SIMPLE(55, log);
	ADDRULE_SIMPLE(59, log);
	ADDRULE_SIMPLE(62, log);
	ADDRULE_SIMPLE(67, log);
	ADDRULE_SIMPLE(73, log);
	ADDRULE_SIMPLE(76, log);
	ADDRULE_SIMPLE(77, log);
	ADDRULE_SIMPLE(78, log);
	ADDRULE_SIMPLE(80, log);
	ADDRULE_SIMPLE(84, log);
	ADDRULE_SIMPLE(85, log);
	ADDRULE_SIMPLE(86, log);
	ADDRULE_SIMPLE(89, log);
	ADDRULE_SIMPLE(99, log);
	ADDRULE_SIMPLE(100, log);
	ADDRULE_SIMPLE(102, log);
	ADDRULE_SIMPLE(103, log);
	ADDRULE_SIMPLE(104, log);
	ADDRULE_SIMPLE(105, log);
	ADDRULE_SIMPLE(106, log);
	ADDRULE_SIMPLE(107, log);
	ADDRULE_SIMPLE(108, log);
	ADDRULE_SIMPLE(109, log);
	ADDRULE_SIMPLE(114, log);
	ADDRULE_SIMPLE(116, log);
	ADDRULE_SIMPLE(117, log);
	ADDRULE_SIMPLE(122, log);
	ADDRULE_SIMPLE(124, log);
	ADDRULE_SIMPLE(126, log);
	ADDRULE_SIMPLE(131, log);
	ADDRULE_SIMPLE(134, log);
	ADDRULE_SIMPLE(135, log);
	ADDRULE_SIMPLE(140, log);
	ADDRULE_SIMPLE(141, log);
	ADDRULE_SIMPLE(142, log);
	ADDRULE_SIMPLE(145, log);
	ADDRULE_SIMPLE(149, log);
	ADDRULE_SIMPLE(155, log);
	ADDRULE_SIMPLE(161, log);
	ADDRULE_SIMPLE(162, log);
	ADDRULE_SIMPLE(165, log);
	ADDRULE_SIMPLE(168, log);
	ADDRULE_SIMPLE(171, log);
	ADDRULE_SIMPLE(172, log);
	ADDRULE_SIMPLE(174, log);
	ADDRULE_SIMPLE(175, log);
	ADDRULE_SIMPLE(176, log);
	ADDRULE_SIMPLE(177, log);
	ADDRULE_SIMPLE(180, log);
	ADDRULE_SIMPLE(183, log);
	ADDRULE_SIMPLE(184, log);
	ADDRULE_SIMPLE(185, log);
	ADDRULE_SIMPLE(187, log);
	ADDRULE_SIMPLE(191, log);
	ADDRULE_SIMPLE(192, log);
	ADDRULE_SIMPLE(195, log);
	ADDRULE_SIMPLE(196, log);
	ADDRULE_SIMPLE(197, log);
	ADDRULE_SIMPLE(205, log);
	ADDRULE_SIMPLE(209, log);
	ADDRULE_SIMPLE(211, log);
	ADDRULE_SIMPLE(218, log);
	ADDRULE_SIMPLE(220, log);
	ADDRULE_SIMPLE(221, log);
	ADDRULE_SIMPLE(229, log);
	ADDRULE_SIMPLE(230, log);
	ADDRULE_SIMPLE(231, log);
	ADDRULE_SIMPLE(232, log);
	ADDRULE_SIMPLE(233, log);
	ADDRULE_SIMPLE(234, log);
	ADDRULE_SIMPLE(239, log);
	ADDRULE_SIMPLE(242, log);
	ADDRULE_SIMPLE(245, log);
	ADDRULE_SIMPLE(247, log);
	ADDRULE_SIMPLE(249, log);
	ADDRULE_SIMPLE(253, log);
	ADDRULE_SIMPLE(256, log);
	ADDRULE_SIMPLE(259, log);
	ADDRULE_SIMPLE(260, log);
	ADDRULE_SIMPLE(261, log);
	ADDRULE_SIMPLE(265, log);
	ADDRULE_SIMPLE(266, log);
	ADDRULE_SIMPLE(267, log);
	ADDRULE_SIMPLE(268, log);
	ADDRULE_SIMPLE(269, log);
	ADDRULE_SIMPLE(275, log);
	ADDRULE_SIMPLE(280, log);
	ADDRULE_SIMPLE(282, log);
	ADDRULE_SIMPLE(284, log);
	ADDRULE_SIMPLE(288, log);
	ADDRULE_SIMPLE(300, log);
	ADDRULE_SIMPLE(305, log);
	ADDRULE_SIMPLE(308, log);
	ADDRULE_SIMPLE(309, log);
	ADDRULE_SIMPLE(312, log);
	ADDRULE_SIMPLE(313, log);
	ADDRULE_SIMPLE(317, log);
	ADDRULE_SIMPLE(318, log);
	ADDRULE_SIMPLE(319, log);
	ADDRULE_SIMPLE(325, log);
	ADDRULE_SIMPLE(326, log);
	ADDRULE_SIMPLE(331, log);
	ADDRULE_SIMPLE(333, log);
	ADDRULE_SIMPLE(337, log);
	ADDRULE_SIMPLE(340, log);
	ADDRULE_SIMPLE(341, log);
	ADDRULE_SIMPLE(343, log);
}
...
#define BR_BUF_SIZE 0x100000

struct buffered_reader *br_open(const char *filename) {
        struct buffered_reader *ret = malloc(sizeof(struct buffered_reader));
        ...
        ret->buf = malloc(sizeof(char*) * BR_BUF_SIZE);
        ret->offset = 0;
        br_read_chunk(ret);
        return ret;
}

int br_close(struct buffered_reader *br) {
        free(br->buf);
        close(br->fd);
        return 0;
}

size_t br_read_chunk(struct buffered_reader *br) {
        int rsize = read(br->fd, (void*)br->buf, BR_BUF_SIZE);
        //debugf("br_read_chunk read %d\n", rsize);
        ...
        br->buf_ptr = br->buf;
        br->buf_end = br->buf_ptr + rsize;
        br->offset += rsize;
        return rsize;
}

size_t br_read(struct buffered_reader *br, void *buf, size_t count) {
        assert(count <= BR_BUF_SIZE);
        if (br->buf_ptr == br->buf_end)
                return 0;

        // there's definitely a more elegant solution:
        if ((size_t)(br->buf_end - br->buf_ptr) >= count) {
                memcpy(buf, br->buf_ptr, count);

                br->buf_ptr += count;
                goto out;
        }
        else {
                int leftover = br->buf_end - br->buf_ptr;
                // copy first chunk:
                memcpy(buf, br->buf_ptr, leftover);
                br_read_chunk(br);
                memcpy(buf + leftover, br->buf_ptr, count - leftover);
                br->buf_ptr += (count - leftover);
        }
...
off_t br_consume(struct buffered_reader *br, off_t offset) {
        //debugf("br_consume %ld %ld %p %p\n", offset, br_offset(br), br->buf_ptr, br->buf_end);

        off_t remaining = offset;
        // consume full reads as necessary
        while ((br->buf_end - br->buf_ptr) <= remaining) {
                remaining -= (br->buf_end - br->buf_ptr);
                br_read_chunk(br);
        }
        br->buf_ptr += remaining;
        return 0;
}

// it's just a pass through
off_t br_offset(struct buffered_reader *br) {
        return br->offset  - (br->buf_end -  br->buf_ptr);
}
