...
void print_help(const char *program) {
	fprintf (stderr, "format: %s <logdir> [-p] [-f] [-m] [-g] [--pthread libdir] [--exit_clock=replay_clock_val] [--attach_offset=pid,sysnum] [--ckpt_at=replay_clock_val] [--from_ckpt=replay_clock-val] [--fake_calls=c1,c2...] \n",
			program);
}
struct ckpt_data { 
    int fd;
...
int get_ckpt_dir() { 
  ...
  fd = open("/proc/sys/kernel/replay_ckpt_dir", O_RDONLY); 
  if (fd < 0) { 
    perror("cannot open replay_ckpt_dir");
    exit (-1); 
  }
  rc = read(fd, ckpt, sizeof(int));   // this sizeof thing is kinda dumb. but whatever
  if (rc <= 0) {
    perror("cannot read from replay_ckpt_dir");
    exit (-1);
  }
  ckpt_val = atoi(ckpt);
  return ckpt_val != 0 ? ckpt_val : 0;    
}
...
int main (int argc, char* argv[])
{
	...
	struct option long_options[] = {
		...
		{"exit_clock", required_argument, 0, 0},
...
#endif
	if (from_ckpt > 0) {
	        int ckpt_dir = get_ckpt_dir();
		if (ckpt_dir) { 
		  sprintf (pathname, "%s/%d/ckpt.%d", argv[base], ckpt_dir, from_ckpt);		  
		  sprintf (filename, "%d/ckpt.%d", ckpt_dir, from_ckpt);
		}
		else { 
		...
		printf ("restoring from %s, kernel sees %s\n", pathname, filename);
...
*/
		...
		rc = resume_with_ckpt (fd, attach_pin, attach_gdb, follow_splits, save_mmap, argv[base], libdir,
				       attach_index, attach_pid, ckpt_at, record_timing, nfake_calls, fake_calls, exit_clock);
	}
	if (rc < 0) {
		perror("resume");
		return -1;
