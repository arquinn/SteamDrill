...
int main (int argc, char* argv[]) 
{
	...
	if (response[l]) {
	    ...
	    fprintf (stderr,"<sending %ld/%d log file...", l, log_files.size());
	...
	if (response[l+j]) {
            char cname[PATHLEN-5], linkname[PATHLEN];
	    struct stat64 st;
	    ...
	    fprintf (stderr,"<send %ld/%d cache file...", j, cache_files.size());
	    // Send the file to streamserver
	    rc = send_file (socket, cname, "rename_me");

            sprintf(linkname, "%s.orig", cname);
            rc = stat64(linkname, &st);
            if (rc < 0) {
                fprintf(stderr, "no mapping file %s, rc=%d\n", cname, rc);
                return rc;
            }
            rc = send_file(socket, linkname, "rename_me");
