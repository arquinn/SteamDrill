...
struct replay_timing {
    ...
    short     syscall;
    u_int     ut;
    uint64_t  st; // ADDED (kinda for nothing)
...
static long read_ckpts (char* dirname)
{
	...
	if (!strncmp(de->d_name, "ckpt.", 5)) {
            rc = snprintf (filename, 80, "%s/%s", dirname, de->d_name);
            if (rc < 0) {
              return rc;
            }
...
//	update_cache_misses(td, total_cache_misses, i); do I even need this? 
    }
...
int main (int argc, char* argv[])
{
    ...
    if(details) std::cout << "coppied " << std::endl;
    rc = parse_klogs(td, stop_clock,argv[1], following, procs);    
    if(details) std::cout << "parsed klogs " << std::endl;
    gettimeofday(&pklog_tv, NULL);


    rc = parse_ulogs(td, argv[1]);     
    if(details) std::cout << "parsed ulogs " << td.size() <<  std::endl;
    gettimeofday(&pulog_tv, NULL);

    rc = parse_timing_data(td);
    if(details) std::cout << "parsed tdata " << std::endl;
    gettimeofday(&ptiming_tv, NULL);


    if(details) {
	for (auto t : td) { 
	    printf ("%d %lu %lu, %lu, (%lf %u %llu), %d\n", t.pid, t.call_clock, t.start_clock, t.stop_clock, t.dtiming, t.ut, t.cache_misses, t.should_track);
