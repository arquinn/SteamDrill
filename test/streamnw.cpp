...
long fetch_file (int s, const char* dest_dir)
{
    ...
    int tries = 0;
    rc = stat (pathname, &check); 
    while (tries <= 10 && !rc && (check.st_mtim.tv_sec != st.st_mtim.tv_sec ||
				  check.st_mtim.tv_nsec != st.st_mtim.tv_nsec)) { 
      ...
      if (rc < 0) {
	fprintf (stderr, "futimens returns %ld for file %s, errno=%d\n", rc, filename, errno);
      }
    ...
    if (check.st_mtim.tv_sec != st.st_mtim.tv_sec ||
	check.st_mtim.tv_nsec != st.st_mtim.tv_nsec) { 
      fprintf(stderr, "%s: never able to get modify time correct!\n", filename);
    }
